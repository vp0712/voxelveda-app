const STATIC_CAPABILITIES = Object.freeze({
  ...
  transaction_split: { status: 'NOT_SUPPORTED', note: 'No canonical split model is implemented yet. UI must not imply otherwise.' },
  refund_linking: { status: 'NOT_SUPPORTED', note: 'Refund candidates may appear in intelligence, but linked refund accounting is not yet implemented.' },
  reimbursements: { status: 'NOT_SUPPORTED', note: 'No dedicated canonical reimbursement lifecycle is implemented yet.' },
  receipt_storage: { status: 'BACKEND_ONLY', note: 'Secure object storage and malware scanning exist; Finance receipt workflow is not yet complete.' },
