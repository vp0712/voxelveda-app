const STATIC_CAPABILITIES = Object.freeze({
  ...
  category_manager: { status: 'PARTIAL', source: 'finance_categories', note: 'Managed system-category metadata, parent categories, GST defaults and archive/restore are implemented; production workflow verification is still required before READY.' },
  ...
  forecast_planning: { status: 'PARTIAL', source: 'Personal Money Smart/Health/Roadmaps + Banking OS', note: 'Forecast, safe-to-spend, roadmap intelligence and scheduled cash-flow events are exposed from existing evidence-backed engines; they never mutate the ledger.' },
  cashflow_calendar: { status: 'PARTIAL', source: 'Banking OS + Personal Money Smart', note: 'Known company/banking and personal planning events are shown separately; production workflow verification is still required.' },
  user_preferences: { status: 'PARTIAL', source: 'finance_user_preferences', note: 'Per-user defaults, formatting and dashboard-card preferences are implemented without changing other users or financial data; production verification is still required.' },
