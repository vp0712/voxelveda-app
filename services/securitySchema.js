...
async function createSecuritySchema() {
  ...
  await tolerateDuplicate('ALTER TABLE users ADD COLUMN employee_number VARCHAR(40) NULL');
  await tolerateDuplicate('ALTER TABLE users ADD COLUMN security_compromised_at DATETIME NULL');
  await tolerateDuplicate('ALTER TABLE users ADD UNIQUE INDEX uq_users_employee_number (employee_number)');
  ...
  await pool.query("UPDATE users SET employee_number = CONCAT('VV-', LPAD(id, 6, '0')) WHERE employee_number IS NULL OR employee_number = ''");
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS user_api_tokens (
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS trusted_devices (
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS user_security_actions (
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS privileged_access_reviews (
