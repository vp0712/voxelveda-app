...
function baseState() {
  return {
    ...
    database: {
      ...
      least_privilege_source: null,
      runtime_user: null,
      runtime_database: null,
      root_identity: null,
      has_grant_option: null,
      unsafe_global_grant: null,
      has_schema_scoped_grant: null
    },
    migrations: {
      state: CONTROL_STATES.NOT_CONFIGURED,
      schema_version: null,
...
function resetRuntimeState() { state = baseState(); return state; }
function setPhase(phase) { state.phase = String(phase || 'UNKNOWN').slice(0, 80); }
function addWarning(warning) { const safe = String(warning || '').trim().slice(0, 500); if (safe && !state.warnings.includes(safe)) state.warnings.push(safe); }
function setDatabase(details = {}) { state.database = { ...state.database, ...details }; }
function setMigrations(details = {}) { state.migrations = { ...state.migrations, ...details }; }
function setCriticalService(key, serviceState, detail = null) { state.critical_services[String(key)] = { state: serviceState, checked_at: new Date().toISOString(), detail: detail ? String(detail).slice(0, 240) : null }; }
function setControl(key, controlState, detail = null) {
  ...
  state.controls[key] = { state: controlState, checked_at: new Date().toISOString(), detail: detail ? String(detail).slice(0, 240) : null };
}
function markReady() { state.ready = true; state.ready_at = new Date().toISOString(); state.phase = 'READY'; state.failure = null; }
