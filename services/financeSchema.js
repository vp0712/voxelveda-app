...
async function createTables() {
  const statements = [
    ...
    `CREATE TABLE IF NOT EXISTS finance_profitability_entities (
      id BIGINT AUTO_INCREMENT PRIMARY KEY,
      ...
      created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
      updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
      UNIQUE KEY uniq_finance_profitability_entity_uid (entity_uid),
      INDEX idx_finance_profitability_entity_status (status,entity_type,name)
    )`,
    `CREATE TABLE IF NOT EXISTS finance_profitability_allocations (
      id BIGINT AUTO_INCREMENT PRIMARY KEY,
      ...
      UNIQUE KEY uniq_finance_profitability_allocation_uid (allocation_uid),
      INDEX idx_profitability_allocation_tx (bank_transaction_id,status),
      INDEX idx_profitability_allocation_entity (entity_id,status,currency,allocation_role)
    )`,
    `CREATE TABLE IF NOT EXISTS finance_profitability_invoice_links (
      id BIGINT AUTO_INCREMENT PRIMARY KEY,
      ...
      reversed_at DATETIME NULL,
      reversal_reason VARCHAR(600) NULL,
      UNIQUE KEY uniq_finance_profitability_invoice_link_uid (link_uid),
      INDEX idx_profitability_invoice_link_invoice (invoice_id,status),
      INDEX idx_profitability_invoice_link_entity (entity_id,status,currency)
    )`,
