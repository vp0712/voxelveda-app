...
async function createOperationalTrustSchema() {
  await tolerateDuplicate('ALTER TABLE user_api_tokens ADD COLUMN token_prefix VARCHAR(24) NULL');
  await tolerateDuplicate('ALTER TABLE user_api_tokens ADD COLUMN created_by INT NULL');
  await tolerateDuplicate('ALTER TABLE user_api_tokens ADD COLUMN last_used_ip VARCHAR(64) NULL');
  await tolerateDuplicate('ALTER TABLE user_api_tokens ADD COLUMN last_used_user_agent VARCHAR(255) NULL');
  await tolerateDuplicate('ALTER TABLE user_api_tokens ADD INDEX idx_api_token_prefix (token_prefix)');

  await pool.query(`CREATE TABLE IF NOT EXISTS webhook_sources (
    id CHAR(36) PRIMARY KEY, source_key VARCHAR(80) NOT NULL UNIQUE, display_name VARCHAR(120) NOT NULL,
    allowed_events_json JSON NOT NULL, active TINYINT(1) NOT NULL DEFAULT 1,
    created_by INT NOT NULL, created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
  ) ENGINE=InnoDB`);
  await pool.query(`CREATE TABLE IF NOT EXISTS webhook_receipts (
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS data_retention_policies (
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS retention_holds (
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS retention_runs (
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS sensitive_export_requests (
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS security_secret_inventory (
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS backup_attestations (
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS malware_scan_jobs (
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS security_alert_rules (
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS security_evidence_snapshots (
...
async function ensureOperationalTrustSchema() {
  if (!schemaPromise) schemaPromise = createOperationalTrustSchema().catch((error) => { schemaPromise = null; throw error; });
  return schemaPromise;
}

module.exports = { ensureOperationalTrustSchema };
