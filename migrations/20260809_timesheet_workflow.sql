-- Voxel Veda controlled timesheet workflow and email delivery infrastructure.
...
DROP PROCEDURE IF EXISTS vv_add_column;
DELIMITER $$
CREATE PROCEDURE vv_add_column(IN table_name_value VARCHAR(64), IN column_name_value VARCHAR(64), IN definition_value TEXT)
BEGIN
  IF NOT EXISTS (
    ...
    SET @column_sql = CONCAT('ALTER TABLE `', table_name_value, '` ADD COLUMN `', column_name_value, '` ', definition_value);
...
CALL vv_add_column('weekly_timesheets', 'ordinary_hours', 'DECIMAL(9,2) NOT NULL DEFAULT 0');
CALL vv_add_column('weekly_timesheets', 'overtime_hours', 'DECIMAL(9,2) NOT NULL DEFAULT 0');
CALL vv_add_column('weekly_timesheets', 'approved_hours', 'DECIMAL(9,2) NOT NULL DEFAULT 0');
CALL vv_add_column('weekly_timesheets', 'submitted_at', 'DATETIME NULL');
CALL vv_add_column('weekly_timesheets', 'approved_at', 'DATETIME NULL');
CALL vv_add_column('weekly_timesheets', 'approved_by', 'INT NULL');
CALL vv_add_column('weekly_timesheets', 'rejected_at', 'DATETIME NULL');
CALL vv_add_column('weekly_timesheets', 'rejected_by', 'INT NULL');
CALL vv_add_column('weekly_timesheets', 'correction_requested_at', 'DATETIME NULL');
CALL vv_add_column('weekly_timesheets', 'correction_requested_by', 'INT NULL');
CALL vv_add_column('weekly_timesheets', 'manager_comments', 'TEXT NULL');
CALL vv_add_column('weekly_timesheets', 'version_no', 'INT NOT NULL DEFAULT 0');
CALL vv_add_column('weekly_timesheets', 'payroll_status', 'VARCHAR(30) NOT NULL DEFAULT ''NOT_READY''');
DROP PROCEDURE vv_add_column;

DROP PROCEDURE IF EXISTS vv_add_index;
DELIMITER $$
CREATE PROCEDURE vv_add_index(IN table_name_value VARCHAR(64), IN index_name_value VARCHAR(64), IN columns_value TEXT)
BEGIN
  IF NOT EXISTS (
    ...
    SET @index_sql = CONCAT('ALTER TABLE `', table_name_value, '` ADD INDEX `', index_name_value, '` (', columns_value, ')');
...
END$$
DELIMITER ;
CALL vv_add_index('weekly_timesheets', 'idx_weekly_timesheet_status', '`status`, `week_start`, `week_end`');
DROP PROCEDURE vv_add_index;
