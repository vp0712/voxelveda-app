...
assert.match(html,/finance-master\.js\?v=20260924-margin-allocation-v1/,'Master finance client must be loaded with the current cache-busting release id.');
...
assert.match(js,/\['profitability','◈','Job Profitability'\]/,'Job Profitability must remain a first-class accounting-based planning module.');
assert.match(js,/\['marginallocation','◩','Margin Allocation'\]/,'Margin Allocation must be a first-class bank-cash planning module.');
assert.match(js,/function marginAllocationView\(\)/,'Margin Allocation canonical view loader is missing.');
assert.match(js,/finance-margin-allocation\.js\?v=20260924-margin-allocation-v1/,'Margin Allocation asset version must be pinned.');
