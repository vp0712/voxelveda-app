...
const client = fs.readFileSync('public/recovery-drill-governance.js', 'utf8');
...
assert(client.includes("/api/security/readiness/recovery/drills/governance"), 'Governance UI must call the protected backend route.');
assert(renderer.includes('/recovery-drill-governance.css'), 'Governance stylesheet must be loaded by the admin renderer.');
assert(renderer.includes('/recovery-drill-governance.js'), 'Governance client must be loaded by the admin renderer.');
