...
assert(!/method\s*:\s*['\"](?:POST|PUT|PATCH|DELETE)/i.test(risk),'Risk Intelligence must remain API read-only.');
...
assert(risk.includes('does not currently have a personal insurance/protection-policy register'),'Risk Intelligence must disclose missing protection-policy source data.');
assert(risk.includes('“No policy data” is <b>not</b> treated as “no insurance”'),'Risk Intelligence must not infer missing insurance coverage from absent records.');
