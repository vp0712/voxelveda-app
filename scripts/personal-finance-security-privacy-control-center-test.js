...
for(const secret of ['manual_key','qr_code','password.value','stepUpPassword'])assert(!ui.includes(secret),`Security Center must not expose secret material: ${secret}`);
assert(!/\.recovery_codes\b|['\"]recovery_codes['\"]/.test(ui),'Security Center must never access or display raw recovery-code material.');
assert(ui.includes('recovery_codes_remaining'),'Security Center may display only the safe remaining recovery-code count.');
