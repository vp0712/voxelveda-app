...
for(const table of ['open_banking_provider_users','open_banking_consent_sessions','open_banking_sync_runs','open_banking_webhook_events'])expect(migration,table,'Missing Open Banking table '+table);
...
expect(ui,'Banking Setup & Safety','Open Banking setup panel missing from unified Finance OS');
expect(ui,'function startBankConsent(provider)','Connect-bank consent action missing');
expect(ui,"I+'/open-banking/consent'",'Finance OS must create consent through the protected endpoint');
expect(ui,'Connect bank','Configured providers must expose the Connect bank action');
expect(ui,'never asks for or stores your bank password, PIN or OTP','Bank credential safety guidance missing from unified Finance OS');
for(const secret of ['BANK_DATA_API_KEY','BASIQ_API_KEY','BANK_DATA_CLIENT_SECRET'])if(ui.includes(secret))throw new Error('Finance UI must not expose provider secret name '+secret);
