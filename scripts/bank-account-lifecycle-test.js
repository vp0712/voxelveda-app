...
const bankingRoutes = fs.readFileSync(path.join(root, 'routes', 'bankingPortalRoutes.js'), 'utf8');
...
assert.match(routes, /\/intelligence\/accounts\/:id\/purge/);
assert.match(bankingRoutes, /\/intelligence\/accounts\/:id\/purge/);
...
assert.match(ui, /data-account-purge/);
assert.match(ui, /Delete account & all data/);
assert.match(ui, /accounts\/'\+encodeURIComponent\(id\)\+'\/purge/);
assert.match(ui, /\['BUSINESS','MIXED'\]\.includes/);
assert.match(ui, /data-team-account-edit/);
assert.match(ui, /Personal accounts remain private/);
...
assert.match(controllerSource, /exports\.purge/);
assert.match(controllerSource, /BANK_ACCOUNT_PURGE_CONFIRMATION_REQUIRED/);
assert.match(controllerSource, /purgeAccountData/);
assert.match(controllerSource, /COLUMN_NAME = \?/);
assert.match(controllerSource, /bank_transaction_id/);
assert.match(controllerSource, /statement_import_rows/);
assert.match(controllerSource, /journal_lines/);
assert.match(controllerSource, /PURGED_WITH_DATA/);
