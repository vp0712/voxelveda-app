(() => {
'use strict';
const ROOT='financeProfitabilityMount',VERSION='20260924-profitability-v1';
...
function entityForm(e=null){
 modal(e?'Edit profitability dimension':'Add profitability dimension',
 ...
 async fd=>{const body=Object.fromEntries(fd.entries());const path=e?'/api/finance/profitability-control/entities/'+encodeURIComponent(e.entity_uid):'/api/finance/profitability-control/entities';const x=await api(path,{method:e?'PUT':'POST',body:JSON.stringify(body)});notice(x.message);await load()});
}
function allocationForm(tx){
 if(!active().length)return notice('Create an active profitability dimension first.',true);
 ...
 async fd=>{const body=Object.fromEntries(fd.entries());body.bank_transaction_id=tx.id;const x=await api('/api/finance/profitability-control/allocations',{method:'POST',body:JSON.stringify(body)});notice(x.message);await load()});
}
function invoiceForm(inv){
 ...
 '<label>Note<textarea name="note"></textarea></label>',
 async fd=>{const body=Object.fromEntries(fd.entries());body.invoice_id=inv.id;const x=await api('/api/finance/profitability-control/invoice-links',{method:'POST',body:JSON.stringify(body)});notice(x.message);await load()});
}
function render(){
 ...
 '<article class="fm-card"><div class="fm-pad"><div class="fm-card-head"><div><h2>Dimensions & margin</h2><p>Native-currency customer/project/service/product profitability.</p></div></div><div class="fm-list">'+(entities||'<div class="fm-empty">No profitability dimensions yet.</div>')+'</div></div></article>'+
...
async function load(){
 ...
 try{data=await api('/api/finance/profitability-control');render()}catch(error){if(root)root.innerHTML='<div class="fm-state fm-state-error"><strong>Profitability Control could not load</strong><p>'+esc(error.message)+'</p><button id="pfcRetry">Retry</button></div>';document.getElementById('pfcRetry')?.addEventListener('click',load)}finally{loading=false}
}
function bind(){
 ...
 document.querySelectorAll('[data-status]').forEach(b=>b.onclick=async()=>{try{const x=await api('/api/finance/profitability-control/entities/'+encodeURIComponent(b.dataset.status)+'/status',{method:'POST',body:JSON.stringify({active:b.dataset.active==='true'})});notice(x.message);await load()}catch(error){notice(error.message,true)}});
 ...
 document.querySelectorAll('[data-reverse]').forEach(b=>b.onclick=async()=>{const reason=prompt('Reversal reason:')||'';if(!reason.trim())return;try{const x=await api('/api/finance/profitability-control/allocations/'+encodeURIComponent(b.dataset.reverse)+'/reverse',{method:'POST',body:JSON.stringify({reason})});notice(x.message);await load()}catch(error){notice(error.message,true)}});
 document.querySelectorAll('[data-unlink]').forEach(b=>b.onclick=async()=>{const reason=prompt('Unlink reason:')||'';if(!reason.trim())return;try{const x=await api('/api/finance/profitability-control/invoice-links/'+encodeURIComponent(b.dataset.unlink)+'/reverse',{method:'POST',body:JSON.stringify({reason})});notice(x.message);await load()}catch(error){notice(error.message,true)}});
}
function mount(){if(document.getElementById(ROOT))load()}
window.__financeProfitabilityMount=mount;
window.addEventListener('finance:profitability-mount',mount);
