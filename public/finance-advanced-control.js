...
const SOURCES=[
  ...
  ['profitability','/api/finance/profitability-control'],
...
function profitabilityMarginControl(){
 const p=state.data.profitability||{},totals=p.totals_by_currency||{},entities=p.entities||[];
 const cards=Object.entries(totals).map(([c,x])=>'<div class="fac-kpi"><span>'+esc(c)+' cash margin</span><b>'+money(x.margin,c)+'</b><small>'+money(x.revenue,c)+' revenue · '+money(x.cost,c)+' cost · '+(x.margin_percent===null?'—':num(x.margin_percent).toFixed(1)+'%')+'</small></div>').join('');
 const below=entities.filter(e=>e.status==='ACTIVE'&&Object.values(e.by_currency||{}).some(x=>e.target_margin_percent!==null&&x.margin_percent!==null&&num(x.margin_percent)<num(e.target_margin_percent))).slice(0,8);
 const rows=below.map(e=>'<div class="fac-row"><div><h4>'+esc(e.name)+'</h4><p>'+esc(e.entity_type)+' · target '+num(e.target_margin_percent).toFixed(1)+'%</p></div><div class="fac-right">'+statusChip('WATCH')+'</div></div>').join('');
 return '<section id="facProfitability" class="fac-section"><header><div><h3>Profitability & Margin Control</h3><p>Explicit BUSINESS bank allocations for cash margin; invoice billing remains separate to prevent double counting.</p></div><button data-fac-open="profitability">Open Profitability</button></header><div class="fac-kpis">'+(cards||'<div class="fac-kpi"><span>Cash margin</span><b>—</b><small>No allocated profitability evidence.</small></div>')+'</div><div class="fac-list" style="margin-top:10px">'+(rows||'<div class="fac-empty">No active dimension is currently below its configured margin target.</div>')+'</div></section>';
}
...
function render(){
 ...
 executive()+actions()+controlActionsSummary()+forecast()+scenario()+decisionIntelligence()+recurringDebt()+riskIntegrity()+taxEvidence()+companyCfo()+treasuryControl()+fpaPlanningControl()+performanceRiskControl()+profitabilityMarginControl()+anomalyExplainability()+accountantHandoverStatus()+automationApprovalControl()+sourceHealth()+
