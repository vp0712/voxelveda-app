...
function actionQueue(){
 ...
 const savings=state.data.savingsReserve?.summary||{};
 if(num(savings.overdue)>0)out.push({severity:'WATCH',title:'Savings target dates overdue',detail:num(savings.overdue)+' active savings/reserve target(s) have passed their recorded target date.',source:'Savings & Reserve'});
 if(num(savings.behind)>0)out.push({severity:'WATCH',title:'Savings funding pace behind recorded target',detail:num(savings.behind)+' active goal(s) are below the calculated pace required for their saved target date.',source:'Savings & Reserve'});
 if(num(savings.emergency_coverage_below_3m)>0)out.push({severity:'WATCH',title:'Emergency reserve coverage under 3.0 months',detail:num(savings.emergency_coverage_below_3m)+' emergency-fund goal(s) are below 3.0 months of recorded Personal Money spending coverage.',source:'Savings & Reserve'});
...
function recurringDebt(){
 ...
 const debts=(state.data.personal?.debts||[]).filter(d=>String(d.status||'').toUpperCase()!=='SETTLED');
 const savings=state.data.savingsReserve||{},goals=savings.goals||[],savingBy=savings.by_currency||{},savingSummary=savings.summary||{};
 const byCur={};
 ...
 const recurringCards=Object.entries(byCur).map(([c,x])=>'<div class="fac-kpi"><span>'+esc(c)+' commitments</span><b>'+money(x.monthly,c)+'/mo</b><small>'+money(x.annual,c)+'/yr · '+x.count+' active outgoing</small></div>').join('');
 const savingCards=Object.entries(savingBy).map(([c,x])=>'<div class="fac-kpi"><span>'+esc(c)+' reserve progress</span><b>'+money(x.current,c)+'</b><small>'+money(x.remaining,c)+' remaining · '+money(x.recent_monthly_pace,c)+'/mo recent pace</small></div>').join('');
 const rRows=recurring.slice(0,12).map(r=>'<div class="fac-row"><div><h4>'+esc(r.name||'Recurring item')+'</h4><p>'+esc(r.item_type||'BILL')+' · '+esc(r.frequency||'')+' · next '+date(r.next_due_date)+(r.renewal_date?' · renewal '+date(r.renewal_date):'')+'</p></div><div class="fac-right"><b>'+money(r.amount,r.currency||'AUD')+'</b>'+statusChip(r.control_status||r.lifecycle_decision||'KEEP')+'</div></div>').join('');
 const dRows=debts.slice(0,12).map(d=>'<div class="fac-row"><div><h4>'+esc(d.counterparty||'Debt')+'</h4><p>'+esc(d.direction||'')+' · due '+date(d.due_date)+'</p></div><div class="fac-right"><b>'+money(d.outstanding_amount,d.currency||'AUD')+'</b>'+statusChip(d.due_date&&String(d.due_date).slice(0,10)<todayIso()?'OVERDUE':d.status||'OPEN')+'</div></div>').join('');
