...
const SOURCES=[
  ...
  ['savingsReserve','/api/finance/personal-money/savings-control'],
...
function actionQueue(){
 ...
 const sr=state.data.savingsReserve?.summary||{};if(num(sr.overdue)>0)out.push({severity:'HIGH',title:'Savings target date overdue',detail:num(sr.overdue)+' active savings goal(s) passed their target date.',source:'Savings & Reserve'});if(num(sr.behind)>0)out.push({severity:'WATCH',title:'Savings pace below target',detail:num(sr.behind)+' active goal(s) are behind the recorded funding pace.',source:'Savings & Reserve'});if(num(sr.emergency_coverage_below_3m)>0)out.push({severity:'WATCH',title:'Emergency reserve coverage below 3 months',detail:num(sr.emergency_coverage_below_3m)+' emergency-fund goal(s) are below three months of recorded Personal Money spending.',source:'Savings & Reserve'});
...
function recurringDebt(){
 ...
 const savings=state.data.savingsReserve||{},goals=savings.goals||[];
 const byCur={};
 ...
 const recurringSummary=Object.entries(byCur).map(([c,x])=>'<div class="fac-kpi"><span>'+esc(c)+' recurring</span><b>'+money(x.monthly,c)+'/mo</b><small>'+money(x.annual,c)+'/yr · '+x.count+' item(s)</small></div>').join('');
 const savingsSummary=Object.entries(savings.by_currency||{}).map(([c,x])=>'<div class="fac-kpi"><span>'+esc(c)+' savings progress</span><b>'+money(x.current,c)+'</b><small>'+money(x.remaining,c)+' remaining · pace '+money(x.recent_monthly_pace,c)+'/mo</small></div>').join('');
 const rRows=recurring.slice(0,16).map(r=>'<div class="fac-row"><div><h4>'+esc(r.name||'Recurring item')+'</h4><p>'+esc(r.item_type||'BILL')+' · '+esc(r.frequency||'')+' · next '+date(r.next_due_date)+(r.renewal_date?' · renewal '+date(r.renewal_date):'')+'</p></div><div class="fac-right"><b>'+money(r.amount,r.currency||'AUD')+'</b>'+statusChip(r.control_status||r.lifecycle_decision||'CLEAR')+'</div></div>').join('');
 const dRows=debts.filter(d=>String(d.status||'').toUpperCase()!=='SETTLED').slice(0,16).map(d=>'<div class="fac-row"><div><h4>'+esc(d.counterparty||'Debt')+'</h4><p>'+esc(d.direction||'')+' · due '+date(d.due_date)+'</p></div><div class="fac-right"><b>'+money(d.outstanding_amount,d.currency||'AUD')+'</b>'+statusChip(d.due_date&&String(d.due_date).slice(0,10)<todayIso()?'OVERDUE':d.status||'OPEN')+'</div></div>').join('');
 const gRows=goals.filter(g=>g.status!=='COMPLETED').slice(0,16).map(g=>'<div class="fac-row"><div><h4>'+esc(g.name||'Savings goal')+'</h4><p>'+esc(String(g.goal_type||'OTHER').replaceAll('_',' '))+' · target '+date(g.target_date)+' · '+esc(g.pace_status||'NO PACE')+'</p></div><div class="fac-right"><b>'+money(g.current_amount,g.currency||'AUD')+' / '+money(g.target_amount,g.currency||'AUD')+'</b>'+statusChip(['BEHIND','OVERDUE'].includes(String(g.pace_status))?'WATCH':g.pace_status||'CLEAR')+'</div></div>').join('');
 return '<section id="facPersonal" class="fac-section"><header><div><h3>Subscription, Debt & Savings Intelligence</h3><p>Recurring commitments, borrowed/lent exposure, reserve funding pace and emergency-fund signals.</p></div><div class="fac-right"><button data-fac-open="recurring">Commitments</button><button data-fac-open="debt">Debt</button><button data-fac-open="savings">Savings</button></div></header><div class="fac-kpis">'+(recurringSummary+savingsSummary||'<div class="fac-kpi"><span>Personal controls</span><b>—</b><small>No recurring or savings control data.</small></div>')+'</div><div class="fac-grid three" style="margin-top:10px"><article class="fac-card"><h4>Recurring / subscriptions</h4><div class="fac-list">'+(rRows||'<div class="fac-empty">No recurring outgoing item.</div>')+'</div></article><article class="fac-card"><h4>Borrowed / lent money</h4><div class="fac-list">'+(dRows||'<div class="fac-empty">No open debt record.</div>')+'</div></article><article class="fac-card"><h4>Savings & reserves</h4><div class="fac-list">'+(gRows||'<div class="fac-empty">No active savings goal.</div>')+'</div></article></div><div class="fac-note">Savings progress and reserve coverage are planning evidence. They do not prove cash is segregated in a bank account and never move money automatically.</div></section>';
}
