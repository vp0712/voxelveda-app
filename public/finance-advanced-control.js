...
const SOURCES=[
  ...
  ['marginAllocation','/api/finance/margin-allocation'],
...
function executiveReadinessBoard(){
 ...
 const dims=[
  ...
  ['Profitability coverage',num(state.data.jobProfitability?.allocation?.unallocated_transaction_count)+num(state.data.marginAllocation?.summary?.unallocated_transaction_count)>0?'WATCH':'CLEAR',num(state.data.jobProfitability?.allocation?.unallocated_transaction_count)+' job-coded gap(s) · '+num(state.data.marginAllocation?.summary?.unallocated_transaction_count)+' bank allocation gap(s)','marginallocation'],
...
function profitabilityCommandControl(){
 const job=state.data.jobProfitability||{},margin=state.data.marginAllocation||{};
 const jc=String(job.currency||'AUD').toUpperCase(),jt=job.totals||{},ja=job.allocation||{},mt=margin.totals_by_currency||{},ms=margin.summary||{};
 const jobCards='<div class="fac-kpi"><span>'+esc(jc)+' job-coded revenue</span><b>'+money(jt.recognized_revenue,jc)+'</b><small>Posted SALE evidence</small></div>'+
  '<div class="fac-kpi"><span>'+esc(jc)+' job direct cost</span><b>'+money(jt.direct_cost,jc)+'</b><small>Expense · supplier bill · payroll</small></div>'+
  '<div class="fac-kpi"><span>'+esc(jc)+' contribution margin</span><b>'+money(jt.contribution_margin,jc)+'</b><small>Accounting job-reference basis</small></div>'+
  '<div class="fac-kpi"><span>Job allocation coverage</span><b>'+num(ja.allocation_coverage_percent).toFixed(1)+'%</b><small>'+num(ja.unallocated_transaction_count)+' economic record(s) unallocated</small></div>';
 const marginCards=Object.entries(mt).map(([c,x])=>'<div class="fac-kpi"><span>'+esc(c)+' allocated cash margin</span><b>'+money(x.margin,c)+'</b><small>'+money(x.revenue,c)+' bank revenue · '+money(x.cost,c)+' bank cost · '+(x.margin_percent===null?'—':num(x.margin_percent).toFixed(1)+'%')+'</small></div>').join('');
 const readiness=num(ja.unallocated_transaction_count)+num(ms.unallocated_transaction_count)+num(ms.below_target_positions);
 return '<section id="facProfitability" class="fac-section"><header><div><h3>Profitability Command</h3><p>Two separate evidence models: posted accounting contribution margin and explicit BUSINESS bank cash-margin allocation.</p></div>'+statusChip(readiness?'WATCH':'CLEAR')+'</header>'+
 '<article class="fac-card"><h4>Job Profitability — accounting evidence</h4><div class="fac-kpis">'+jobCards+'</div><div class="fac-toolbar"><button data-fac-open="profitability">Open Job Profitability</button></div></article>'+
 '<article class="fac-card"><h4>Margin Allocation — bank cash evidence</h4><div class="fac-kpis">'+(marginCards||'<div class="fac-kpi"><span>Cash margin</span><b>—</b><small>No explicit BUSINESS bank allocation yet.</small></div>')+'</div><div class="fac-kpis" style="margin-top:10px"><div class="fac-kpi"><span>Margin dimensions</span><b>'+num(ms.active_dimensions)+'</b><small>Customer / project / service / product / channel</small></div><div class="fac-kpi"><span>Below target</span><b>'+num(ms.below_target_positions)+'</b><small>Configured target positions</small></div><div class="fac-kpi"><span>Unallocated bank rows</span><b>'+num(ms.unallocated_transaction_count)+'</b><small>Eligible BUSINESS bank evidence</small></div><div class="fac-kpi"><span>Unlinked invoices</span><b>'+num(ms.unlinked_invoice_count)+'</b><small>Billing context not assigned</small></div></div><div class="fac-toolbar"><button data-fac-open="marginallocation">Open Margin Allocation</button></div></article>'+
 '<div class="fac-note">Do not add these two margin models together. Job Profitability uses posted accounting transactions in the configured accounting currency; Margin Allocation uses explicit BUSINESS bank cash evidence in each native currency. Invoice billing remains context only and is not added to bank cash margin.</div></section>';
}
