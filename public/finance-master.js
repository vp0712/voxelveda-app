...
function title(v){return ({
 ...
 forecast:['Forecast','Evidence-based planning from Personal Money Smart, Health and Roadmaps. Forecasts never mutate the ledger.'],
 calendar:['Cash Flow Calendar','Known company/banking and personal planning events shown separately without claiming bank execution.'],
 ...
 rules:['Rules','Merchant categorisation rules create suggestions; they do not silently post changes.'],
...
function dashboardCardVisible(key){const configured=state.userPreferences?.dashboard_cards||[];return key==='attention'||!configured.length||configured.includes(key)}
...
function rulesView(){
 ...
 const list=rows.map(x=>'<div class="fm-row"><div><h3>'+esc(x.merchant_pattern)+'</h3><p>'+esc(x.category||'No category')+' · '+esc(x.ownership_scope||'Any scope')+' · priority '+num(x.priority)+'</p></div><div class="fm-row-right">'+statusBadge(x.enabled?'ACTIVE':'DISABLED')+'<button data-rule-edit="'+x.id+'">Edit</button><button data-rule-delete="'+x.id+'">Delete</button></div></div>').join('');
 return resourceError('rules','Finance Rules')+'<article class="fm-card"><div class="fm-pad"><div class="fm-card-head"><div><h2>Merchant & Category Rules</h2><p>Rules create suggestions only; they never silently post, reconcile or alter source evidence.</p></div><button id="runAnalysis" type="button">Analyse transactions</button></div><div class="fm-list">'+(list||emptyState('No saved rules','Approve or remember a transaction classification to create a merchant rule.'))+'</div></div></article>';
}
function categoriesView(){
 const categories=state.categories?.categories||[],active=categories.filter(x=>Number(x.active)&&!x.archived_at),archived=categories.filter(x=>!Number(x.active)||x.archived_at);
 const activeRows=active.map(x=>'<div class="fm-row"><div><h3>'+(x.icon?esc(x.icon)+' ':'')+esc(x.name)+'</h3><p>'+(x.parent_name?'Under '+esc(x.parent_name)+' · ':'')+esc(x.scope)+' · GST '+esc(x.gst_default||'REVIEW')+(x.color?' · '+esc(x.color):'')+'</p></div><div class="fm-row-right"><button data-category-edit="'+x.id+'">Edit</button><button data-category-archive="'+x.id+'">Archive</button></div></div>').join('');
 const archivedRows=archived.map(x=>'<div class="fm-row"><div><h3>'+esc(x.name)+'</h3><p>'+esc(x.scope)+' · archived '+date(x.archived_at)+'</p></div><button data-category-restore="'+x.id+'">Restore</button></div>').join('');
 return resourceError('categories','Finance Categories')+'<article class="fm-card"><div class="fm-pad"><div class="fm-card-head"><div><h2>System Categories</h2><p>Current/system classification metadata. Original bank categories remain immutable source evidence.</p></div><button id="addFinanceCategory">+ Category</button></div><div class="fm-list">'+(activeRows||emptyState('No system categories','Create categories for Personal, Company or Both.'))+'</div>'+(archivedRows?'<details class="fm-removed-statements"><summary>Archived categories <span>'+archived.length+'</span></summary><div class="fm-list">'+archivedRows+'</div></details>':'')+'<p class="fm-helper">GST defaults are review defaults only. Raw bank terminology is never promoted automatically into authoritative accounting or tax classification.</p></div></article>';
}
...
function bindDynamic(){
 ...
 if($('financePreferencesForm'))$('financePreferencesForm').onsubmit=async e=>{
  e.preventDefault();const fd=new FormData(e.currentTarget);
  const body={default_workspace:fd.get('default_workspace'),default_account_id:fd.get('default_account_id')||null,reporting_currency:String(fd.get('reporting_currency')||'').toUpperCase()||null,default_period:fd.get('default_period'),dashboard_cards:fd.getAll('dashboard_cards'),date_format:fd.get('date_format'),number_format:fd.get('number_format')};
  try{const x=await api(API+'/preferences',{method:'PUT',body:JSON.stringify(body)});state.userPreferences=x.preferences;notice(x.message);render()}catch(error){notice(error.message,true)}
 };
