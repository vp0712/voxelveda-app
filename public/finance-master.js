...
const state={
  ...
  transferCandidates:null,refundCandidates:null,reimbursements:null,briefing:null,savedViews:null,bankingBudgets:null,notifications:null,notificationPrefs:null,companySettings:null,receiptCenter:null,savedReports:null,reportResult:null,archivedTransactions:null,cashflowCalendar:null,accountingPeriods:null,categories:null,smart:null,health:null,roadmaps:null,netWorth:null,assetLifecycle:null,userPreferences:null,preferencesApplied:false,companySummary:null,openBankProviders:null,openBankSessions:null,bankConnectionData:null,bankSyncJobs:null,personalBankDash:null,businessBankDash:null,bankingOps:null,fxRates:null,cashControl:null,debtPlanner:null,closeAssurance:null,treasuryControl:null,handover:null,
...
const NAV_GROUPS=[
 ...
 ['PLANNING',[['budgets','◫','Budgets'],['savings','◎','Savings Goals'],['networth','◇','Net Worth'],['forecast','◷','Forecast'],['calendar','▦','Cash Flow Calendar'],['treasury','▥','Treasury']]],
...
async function hydrateSupplementary(cycle){
 ...
 const resources=[
  ...
  ['cashflowCalendar',OS+'/cashflow-calendar?days=90'],['cashControl',API+'/cash-control'],['closeAssurance',API+'/close-assurance'],['treasuryControl',API+'/treasury-control'],['handover',API+'/accountant-handover'],['accountingPeriods',API+'/accounting-periods'],['categories',API+'/categories?include_archived=true'],['smart',API+'/personal-money/smart'],['health',API+'/personal-money/health'],
...
function bindDynamic(){
 ...
 document.querySelectorAll('[data-lifecycle-document-delete]').forEach(b=>b.onclick=async()=>{if(!confirm('Remove this document reference? The external file itself will not be deleted.'))return;try{const x=await api(API+'/personal-money/net-worth/lifecycle/documents/'+encodeURIComponent(b.dataset.lifecycleDocumentDelete),{method:'DELETE'});notice(x.message);await refresh();state.view='networth';render()}catch(error){notice(error.message,true)}});
