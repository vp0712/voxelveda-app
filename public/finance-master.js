...
const state={
  ...
  transferCandidates:null,refundCandidates:null,reimbursements:null,briefing:null,savedViews:null,bankingBudgets:null,notifications:null,notificationPrefs:null,companySettings:null,receiptCenter:null,savedReports:null,reportResult:null,archivedTransactions:null,smart:null,health:null,roadmaps:null,cashflowCalendar:null,categories:null,userPreferences:null,preferencesApplied:false,
...
function title(v){return ({
 ...
 forecast:['Forecast','Evidence-based planning from Smart Money, Financial Health and Roadmap data without mutating the ledger.'],
 calendar:['Cash Flow Calendar','Upcoming known cash-flow events kept separate by source, scope and currency.'],
...
async function loadBase(){
 ...
 state.smart=smart||null;state.health=health||null;state.roadmaps=roadmaps||null;state.cashflowCalendar=cashflowCalendar||null;state.categories=categories||null;
...
function overview(){
 ...
 const analytics=cashflowCard+expenseCard?'<div class="fm-grid two">'+cashflowCard+expenseCard+'</div>':'';
 const operations='<div class="fm-grid two">'+attentionCard+accountCard+'</div>';
 return hero()+analytics+operations+recentCard;
}
function accounts(){
...
function settingsView(){
 ...
 return company+userPreferencesCard()+connectionsView()+'<article class="fm-card"><div class="fm-pad"><div class="fm-card-head"><div><h2>Capability Registry</h2><p>Unsupported workflows are explicitly identified rather than presented as dead buttons.</p></div></div><div class="fm-capability-grid">'+Object.entries(caps).map(([k,v])=>'<div class="fm-capability"><div><b>'+esc(k.replaceAll('_',' '))+'</b><p>'+esc(v.note||'')+'</p></div>'+statusBadge(v.status)+'</div>').join('')+'</div></div></article>';
}
...
function forecastView(){
 ...
 return smartErr+healthErr+roadErr+
  '<article class="fm-card"><div class="fm-pad"><div class="fm-card-head"><div><h2>Forecast & Safe-to-Spend</h2><p>Planning estimates use existing Personal Money evidence. Expected income is not counted as available cash, and no forecast mutates financial records.</p></div></div><div class="fm-grid four">'+(safeCards||'<div class="fm-kpi"><span>Safe to spend</span><strong>—</strong><small>No personal planning balance is available.</small></div>')+'</div></div></article>'+
...
function calendarView(){
 ...
 const businessRows=business.map(e=>'<div class="fm-row"><div><h3>'+esc(e.title||'Payment')+'</h3><p>'+date(e.date)+' · '+esc(e.status||'')+' · '+esc(e.account_name||'No account')+'</p></div><div class="fm-row-right"><b>'+nativeMoney(e.amount,e.currency)+'</b><small>'+esc(e.type||'PAYMENT')+'</small></div></div>').join('');
 const personalRows=personal.map(e=>'<div class="fm-row"><div><h3>'+esc(e.name||e.counterparty||e.title||'Personal item')+'</h3><p>'+date(e.date||e.due_date)+' · '+esc(e.frequency||e.type||'')+'</p></div><div class="fm-row-right"><b>'+nativeMoney(e.amount,e.currency)+'</b><small>'+esc(e.direction||'')+'</small></div></div>').join('');
 return resourceError('cashflowCalendar','Cash Flow Calendar')+resourceError('smart','Personal Money Calendar')+
 '<div class="fm-grid two"><article class="fm-card"><div class="fm-pad"><div class="fm-card-head"><div><h2>Company / Banking Calendar</h2><p>Known payment workflow events for the next 90 days. This does not imply bank payment execution.</p></div></div><div class="fm-list">'+(businessRows||emptyState('No upcoming banking events','No visible pending payment events were returned.'))+'</div></div></article>'+
 '<article class="fm-card"><div class="fm-pad"><div class="fm-card-head"><div><h2>Personal Money Calendar</h2><p>Approved recurring reminders and detected planning events remain separate from Company Finance.</p></div></div><div class="fm-list">'+(personalRows||emptyState('No personal events','No known Personal Money events are due in the planning window.'))+'</div></div></article></div>';
}
function categoriesView(){
 const managed=state.categories?.categories||[],observed=state.categories?.observed_categories||[];
 const managedRows=managed.map(c=>'<div class="fm-row"><div><h3>'+esc(c.name)+'</h3><p>'+esc(c.scope)+' · '+(c.parent_name?'under '+esc(c.parent_name)+' · ':'')+'GST '+esc(c.gst_default)+'</p></div><div class="fm-row-right">'+statusBadge(Number(c.active)?'ACTIVE':'ARCHIVED')+'<small>'+(c.colour?'<span class="fm-colour-dot" style="background:'+esc(c.colour)+'"></span>':'')+esc(c.icon||'')+'</small><button data-category-edit="'+c.id+'">Edit</button>'+(Number(c.active)?'<button data-category-archive="'+c.id+'">Archive</button>':'<button data-category-restore="'+c.id+'">Restore</button>')+'</div></div>').join('');
 const known=new Set(managed.map(c=>String(c.name).toLowerCase()));
 const observedRows=observed.filter(c=>!known.has(String(c.category||'').toLowerCase())).slice(0,80).map(c=>'<div class="fm-row"><div><h3>'+esc(c.category)+'</h3><p>Observed current classification · '+esc(c.ownership_scope)+' · '+num(c.transaction_count)+' transaction(s) · last used '+date(c.last_used)+'</p></div><button data-category-promote="'+esc(c.category)+'">Manage</button></div>').join('');
 return resourceError('categories','Categories')+
 '<div class="fm-grid two"><article class="fm-card"><div class="fm-pad"><div class="fm-card-head"><div><h2>System Categories</h2><p>Manage current classification metadata. Original bank categories remain immutable source evidence.</p></div><button id="addFinanceCategory">+ Category</button></div><div class="fm-list">'+(managedRows||emptyState('No managed categories','Create a managed category or promote one observed in your ledger.'))+'</div></div></article>'+
 '<article class="fm-card"><div class="fm-pad"><div class="fm-card-head"><div><h2>Observed Classifications</h2><p>Existing transaction categories not yet managed here.</p></div></div><div class="fm-list">'+(observedRows||emptyState('No unmanaged categories','Current classifications are already managed or no categories exist.'))+'</div></div></article></div>';
}
function userPreferencesCard(){
 const p=state.userPreferences||{},cards=Array.isArray(p.dashboard_cards)?p.dashboard_cards:[];
 const cardChecked=(key)=>!cards.length||cards.includes(key);
 return '<article class="fm-card"><div class="fm-pad"><div class="fm-card-head"><div><h2>User Preferences</h2><p>Personal defaults only. These settings never change another user\'s Finance workspace.</p></div></div><form id="financePreferencesForm" class="fm-form"><div class="fm-form-grid"><label>Default workspace<select name="default_workspace"><option value="ALL" '+(p.default_workspace==='ALL'?'selected':'')+'>Consolidated</option><option value="PERSONAL" '+(p.default_workspace==='PERSONAL'?'selected':'')+'>Personal</option><option value="BUSINESS" '+(p.default_workspace==='BUSINESS'?'selected':'')+'>Company</option></select></label><label>Default account<select name="default_account_id"><option value="">All permitted accounts</option>'+state.accounts.map(a=>'<option value="'+a.id+'" '+(String(p.default_account_id||'')===String(a.id)?'selected':'')+'>'+esc(a.nickname||'Account')+' · '+esc(a.currency||'AUD')+'</option>').join('')+'</select></label><label>Default period<select name="default_period">'+[['month','This Month'],['last_month','Last Month'],['last30','Last 30 Days'],['quarter','This Quarter'],['fy','Current Financial Year'],['year','Calendar Year']].map(([v,l])=>'<option value="'+v+'" '+(p.default_period===v?'selected':'')+'>'+l+'</option>').join('')+'</select></label><label>Preferred reporting currency<input name="reporting_currency" maxlength="3" value="'+esc(p.reporting_currency||'')+'" placeholder="AUD"></label><label>Date format<select name="date_format"><option '+(p.date_format==='DD/MM/YYYY'?'selected':'')+'>DD/MM/YYYY</option><option '+(p.date_format==='MM/DD/YYYY'?'selected':'')+'>MM/DD/YYYY</option><option '+(p.date_format==='YYYY-MM-DD'?'selected':'')+'>YYYY-MM-DD</option></select></label><label>Number format<select name="number_format"><option value="en-AU" '+(p.number_format==='en-AU'?'selected':'')+'>Australia</option><option value="en-US" '+(p.number_format==='en-US'?'selected':'')+'>United States</option><option value="en-GB" '+(p.number_format==='en-GB'?'selected':'')+'>United Kingdom</option></select></label></div><fieldset><legend>Overview cards</legend><label class="fm-check"><input type="checkbox" name="dashboard_cards" value="cashflow" '+(cardChecked('cashflow')?'checked':'')+'> Cash Flow Trend</label><label class="fm-check"><input type="checkbox" name="dashboard_cards" value="expenses" '+(cardChecked('expenses')?'checked':'')+'> Expense Distribution</label><label class="fm-check"><input type="checkbox" name="dashboard_cards" value="accounts" '+(cardChecked('accounts')?'checked':'')+'> Accounts</label><label class="fm-check"><input type="checkbox" name="dashboard_cards" value="recent" '+(cardChecked('recent')?'checked':'')+'> Recent Transactions</label><p class="fm-helper">Needs Your Attention remains visible for safety and data-quality reasons.</p></fieldset><p class="fm-helper">Reporting-currency conversion is not activated unless verified FX evidence exists; this preference does not relabel native amounts.</p><div class="fm-form-actions"><button class="primary" type="submit">Save preferences</button></div></form></div></article>';
}
function openCategoryForm(category=null,prefill=''){
 const c=category||{},active=(state.categories?.categories||[]).filter(x=>Number(x.active)&&String(x.id)!==String(c.id||''));
 $('fmModalEyebrow').textContent='FINANCE CATEGORIES';$('fmModalTitle').textContent=c.id?'Edit category':'New category';
 $('fmModalBody').innerHTML='<form id="financeCategoryForm" class="fm-form"><label>Name<input name="name" value="'+esc(c.name||prefill)+'" required></label><div class="fm-form-grid"><label>Scope<select name="scope"><option '+(c.scope==='PERSONAL'?'selected':'')+'>PERSONAL</option><option '+(c.scope==='BUSINESS'?'selected':'')+'>BUSINESS</option><option '+(!c.scope||c.scope==='BOTH'?'selected':'')+'>BOTH</option></select></label><label>Parent<select name="parent_category_id"><option value="">None</option>'+active.map(p=>'<option value="'+p.id+'" '+(String(c.parent_category_id||'')===String(p.id)?'selected':'')+'>'+esc(p.name)+'</option>').join('')+'</select></label><label>GST default<select name="gst_default">'+['REVIEW','GST','GST_FREE','INPUT_TAXED','NOT_APPLICABLE'].map(v=>'<option '+(c.gst_default===v?'selected':'')+'>'+v+'</option>').join('')+'</select></label><label>Colour<input name="colour" type="color" value="'+esc(c.colour||'#6b7280')+'"></label><label>Icon<input name="icon" value="'+esc(c.icon||'')+'" maxlength="40" placeholder="Optional short label/icon"></label></div><div class="fm-form-actions"><button type="button" data-modal-cancel="1">Cancel</button><button class="primary" type="submit">Save category</button></div></form>';
 $('fmModal').showModal();document.querySelector('[data-modal-cancel]')?.addEventListener('click',()=>$('fmModal').close());
 $('financeCategoryForm').onsubmit=async e=>{e.preventDefault();const fd=new FormData(e.currentTarget),body=Object.fromEntries(fd.entries());body.parent_category_id=body.parent_category_id?Number(body.parent_category_id):null;try{const x=await api(API+'/categories'+(c.id?'/'+c.id:''),{method:c.id?'PUT':'POST',body:JSON.stringify(body)});$('fmModal').close();notice(x.message);await refresh();state.view='categories';render()}catch(error){notice(error.message,true)}};
}
function simpleView(v){
 ...
 if(v==='categories')return categoriesView();
...
function bindDynamic(){
 ...
 if($('addFinanceCategory'))$('addFinanceCategory').onclick=()=>openCategoryForm();
 document.querySelectorAll('[data-category-edit]').forEach(b=>b.onclick=()=>{const c=(state.categories?.categories||[]).find(x=>String(x.id)===String(b.dataset.categoryEdit));if(c)openCategoryForm(c)});
 document.querySelectorAll('[data-category-promote]').forEach(b=>b.onclick=()=>openCategoryForm(null,b.dataset.categoryPromote));
 document.querySelectorAll('[data-category-archive]').forEach(b=>b.onclick=async()=>{if(!confirm('Archive this category? Existing transaction classifications will be preserved.'))return;try{const x=await api(API+'/categories/'+encodeURIComponent(b.dataset.categoryArchive)+'/archive',{method:'POST',body:'{}'});notice(x.message);await refresh();state.view='categories';render()}catch(error){notice(error.message,true)}});
 document.querySelectorAll('[data-category-restore]').forEach(b=>b.onclick=async()=>{try{const x=await api(API+'/categories/'+encodeURIComponent(b.dataset.categoryRestore)+'/restore',{method:'POST',body:'{}'});notice(x.message);await refresh();state.view='categories';render()}catch(error){notice(error.message,true)}});
...
async function transactionDetail(id){
 try{
  ...
  setTimeout(()=>{
   ...
   document.querySelectorAll('[data-refund-link]').forEach(b=>b.onclick=()=>openRefundLink(b.dataset.refundLink,b.dataset.refundCurrency));
   document.querySelector('[data-transaction-archive]')?.addEventListener('click',async()=>{const reason=prompt('Archive reason:');if(!reason)return;try{const x=await api(API+'/bank-transactions/'+id+'/archive',{method:'POST',body:JSON.stringify({reason})});notice(x.message);closeDrawer();await refresh()}catch(error){notice(error.message,true)}});
