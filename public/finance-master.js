...
async function loadBase(){
 ...
 if(tx){state.tx=tx.transactions||[];state.txMeta={page:num(tx.page)||1,limit:num(tx.limit)||50,total:num(tx.total),total_pages:num(tx.total_pages)||1,summary:tx.summary||{},summary_by_currency:tx.summary_by_currency||[],filters:tx.filters||{}}}
 else{state.tx=[]}
...
function transactions(){
 ...
 const categoryContext=f.category?(()=>{
  const sums=Array.isArray(meta.summary_by_currency)?meta.summary_by_currency:[];
  const totals=sums.length?sums.map(row=>'<span><b>'+nativeMoney(row.money_out||0,row.currency||f.currency||'AUD')+'</b><small>'+esc(row.currency||'')+' spend</small></span>').join(''):'<span><b>'+num(meta.total)+'</b><small>matching transactions</small></span>';
  return '<div class="fm-category-drilldown-summary"><div><span class="fm-section-kicker">CATEGORY DRILLDOWN</span><strong>'+esc(f.category)+'</strong><small>'+(state.account?esc(state.accounts.find(a=>String(a.id)===String(state.account))?.nickname||'Selected account'):'All permitted accounts')+(f.currency?' · '+esc(f.currency):'')+'</small></div><div class="fm-category-drilldown-totals"><span><b>'+num(meta.total)+'</b><small>transactions</small></span>'+totals+'</div></div>';
 })():'';
...
async function openStatementVaultEditor(uid){
 try{
  ...
  const rows=transactions.map(tx=>`<div class="fm-row fm-vault-editor-row"><div class="fm-vault-editor-main"><h3>${esc(tx.merchant_name||tx.description||'Transaction')}</h3><p>${date(tx.transaction_date)} · ${esc(tx.category||'Uncategorised')} · ${esc(tx.reconciliation_status||'')}</p></div><div class="fm-row-right fm-vault-editor-side"><b class="fm-vault-editor-amount">${nativeMoney(Math.abs(num(tx.credit)-num(tx.debit)),tx.currency||statement.currency||'AUD')}</b><div class="fm-inline-actions fm-vault-editor-actions"><button type="button" data-statement-correct-tx="${esc(tx.id)}">Edit transaction</button><button type="button" data-statement-classify-tx="${esc(tx.id)}">Classification</button></div></div></div>`).join('');
...
function counterpartyControlView(){
 const d=state.counterpartyControl||{},ar=d.receivables||{},ap=d.payables||{},c=d.currency||'AUD';
 const customerAging=ar.aging||{},supplierAging=ap.aging||{},people=d.counterparties||[],invoices=d.customer_invoices||[],bills=d.supplier_bills||[];
 const ageCards=[['0–30 days',customerAging['0_30']],['31–60 days',customerAging['31_60']],['61–90 days',customerAging['61_90']],['90+ days',customerAging['90_PLUS']]].map(([label,x])=>'<div class="fm-kpi"><span>AR '+label+'</span><strong>'+nativeMoney(x?.amount||0,c)+'</strong><small>'+num(x?.count)+' invoice(s)</small></div>').join('');
 const payCards=[['Overdue',supplierAging.OVERDUE],['Due ≤7 days',supplierAging.DUE_7],['Due ≤30 days',supplierAging.DUE_30],['Future',supplierAging.FUTURE]].map(([label,x])=>'<div class="fm-kpi"><span>AP '+label+'</span><strong>'+nativeMoney(x?.amount||0,c)+'</strong><small>'+num(x?.count)+' bill(s)</small></div>').join('');
 const peopleRows=people.slice(0,40).map(p=>'<div class="fm-row"><div><h3>'+esc(p.name||'Counterparty')+'</h3><p>'+esc(p.party_type)+' · '+num(p.record_count)+' document(s)'+(p.email?' · '+esc(p.email):'')+'</p></div><div class="fm-row-right"><b>'+nativeMoney(p.open_amount,c)+' open</b><small>'+nativeMoney(p.paid_amount,c)+' paid/received · '+nativeMoney(p.total_value,c)+' total</small></div></div>').join('');
 const invoiceRows=invoices.filter(x=>num(x.balance_due)>0.009).slice(0,30).map(x=>'<div class="fm-row"><div><h3>'+esc(x.customer_name||'Customer')+' · '+esc(x.invoice_no||('Invoice #'+x.id))+'</h3><p>'+num(x.age_days)+' day(s) since invoice creation · '+esc(x.payment_state||x.status||'')+'</p></div><div class="fm-row-right"><b>'+nativeMoney(x.balance_due,c)+'</b><small>'+nativeMoney(x.paid_amount,c)+' received of '+nativeMoney(x.total,c)+'</small><div class="fm-inline-actions"><button data-customer-invoice="'+x.id+'">Open invoice</button><button data-customer-statement-name="'+esc(x.customer_name||'')+'" data-customer-statement-email="'+esc(x.customer_email||'')+'">Statement</button></div></div></div>').join('');
 const billRows=bills.filter(x=>num(x.balance_due)>0.009&&!['PAID','VOID'].includes(String(x.status||'').toUpperCase())).slice(0,30).map(x=>'<div class="fm-row"><div><h3>'+esc(x.supplier_name||'Supplier')+' · '+esc(x.supplier_invoice_no||x.bill_uid||('Bill #'+x.id))+'</h3><p>'+esc(String(x.due_bucket||'').replaceAll('_',' '))+' · due '+date(x.due_date)+' · '+esc(x.status||'')+'</p></div><div class="fm-row-right"><b>'+nativeMoney(x.balance_due,c)+'</b><small>'+nativeMoney(x.paid_amount,c)+' paid of '+nativeMoney(x.total_amount,c)+'</small><button data-company-bill="'+x.id+'">Open bill</button></div></div>').join('');
 return resourceError('counterpartyControl','Customers & Suppliers')+
 '<div class="fm-control-intro"><p>AR / AP & COUNTERPARTY CONTROL</p><h2>Customers, suppliers, collections pressure and payment exposure in one place</h2><span>Customer invoice aging uses invoice creation age because the legacy invoice ledger has no due-date field. Supplier aging uses recorded due dates. No foreign currency is invented.</span><div class="fm-control-quick"><button data-viewjump="treasury">Treasury</button><button data-viewjump="company">Company Finance</button><button data-viewjump="reports">Reports</button></div></div>'+
 '<div class="fm-grid four"><div class="fm-kpi"><span>Open receivables</span><strong>'+nativeMoney(ar.open_balance||0,c)+'</strong><small>'+num(ar.open_count)+' customer invoice(s)</small></div><div class="fm-kpi"><span>Open payables</span><strong>'+nativeMoney(ap.open_balance||0,c)+'</strong><small>'+num(ap.open_count)+' supplier bill(s)</small></div><div class="fm-kpi"><span>Top customer concentration</span><strong>'+num(ar.concentration?.percent).toFixed(1)+'%</strong><small>'+esc(ar.concentration?.top_name||'No open customer balance')+'</small></div><div class="fm-kpi"><span>Top supplier concentration</span><strong>'+num(ap.concentration?.percent).toFixed(1)+'%</strong><small>'+esc(ap.concentration?.top_name||'No open supplier balance')+'</small></div></div>'+
 '<article class="fm-card"><div class="fm-pad"><div class="fm-card-head"><div><h2>Receivables age</h2><p>Age since invoice creation; not represented as contractual overdue unless a due date exists elsewhere.</p></div></div><div class="fm-grid four">'+ageCards+'</div></div></article>'+
 '<article class="fm-card"><div class="fm-pad"><div class="fm-card-head"><div><h2>Payables due profile</h2><p>Supplier obligations grouped from recorded bill due dates.</p></div></div><div class="fm-grid four">'+payCards+'</div></div></article>'+
 '<div class="fm-grid two"><article class="fm-card"><div class="fm-pad"><div class="fm-card-head"><div><h2>Customer collection queue</h2><p>Open invoice balances. Record payment from the protected invoice workflow.</p></div></div><div class="fm-list">'+(invoiceRows||emptyState('No open receivables','There are no unpaid/partially paid customer invoices.'))+'</div></div></article><article class="fm-card"><div class="fm-pad"><div class="fm-card-head"><div><h2>Supplier payment queue</h2><p>Open supplier balances. Payment execution remains behind existing approval/step-up controls.</p></div></div><div class="fm-list">'+(billRows||emptyState('No open payables','There are no open supplier bills.'))+'</div></div></article></div>'+
 '<article class="fm-card"><div class="fm-pad"><div class="fm-card-head"><div><h2>Counterparty exposure</h2><p>Customer and supplier balances ranked by current open amount.</p></div></div><div class="fm-list">'+(peopleRows||emptyState('No counterparties','Customer invoices and supplier bills will populate this control surface.'))+'</div></div></article>'+
 '<div class="fm-state"><strong>Currency boundary</strong><p>'+esc(d.currency_note||'Counterparty documents use Company base currency; bank FX remains separate.')+'</p></div>';
}
function jobProfitabilityView(){
 ...
 '<div class="fm-grid four"><div class="fm-kpi"><span>Job-coded revenue</span><strong>'+nativeMoney(t.recognized_revenue,c)+'</strong><small>SALE net amounts</small></div><div class="fm-kpi"><span>Job-coded direct cost</span><strong>'+nativeMoney(t.direct_cost,c)+'</strong><small>Operating direct-cost types</small></div><div class="fm-kpi"><span>Contribution margin</span><strong class="'+(num(t.contribution_margin)<0?'bad':'good')+'">'+nativeMoney(t.contribution_margin,c)+'</strong><small>Before unallocated overhead</small></div><div class="fm-kpi"><span>Allocation coverage</span><strong>'+num(a.allocation_coverage_percent).toFixed(1)+'%</strong><small>'+num(a.unallocated_transaction_count)+' economic record(s) unallocated</small></div></div>'+
 '<div class="fm-grid two"><article class="fm-card"><div class="fm-pad"><div class="fm-card-head"><div><h2>Allocation control</h2><p>Do not hide overhead or guess project ownership.</p></div></div><div class="fm-detail-grid"><span>Unallocated revenue<b>'+nativeMoney(a.unallocated_revenue,c)+'</b></span><span>Unallocated direct cost<b>'+nativeMoney(a.unallocated_direct_cost,c)+'</b></span><span>Economic records<b>'+num(a.economic_transaction_count)+'</b></span><span>Allocated to jobs<b>'+num(a.allocated_transaction_count)+'</b></span></div><p class="fm-helper">'+esc(data.overhead_rule||'Unallocated overhead is not assigned silently.')+'</p></div></article><article class="fm-card"><div class="fm-pad"><div class="fm-card-head"><div><h2>Calculation boundary</h2><p>Why the margin does not double count cash movements.</p></div></div><p class="fm-helper">'+esc(data.margin_rule||'')+'</p><p class="fm-helper">'+esc(data.exclusion_rule||'')+'</p></div></article></div>'+
 '<section class="fm-list">'+(rows||emptyState('No job-coded posted activity','Add a job reference to posted economic Finance transactions to build project profitability.'))+'</section>'+
 '<article class="fm-card"><div class="fm-pad"><div class="fm-card-head"><div><h2>Unallocated economic evidence</h2><p>Posted revenue/direct-cost records without a job reference reduce allocation confidence.</p></div><button data-viewjump="controlactions">Create / manage control actions</button></div><div class="fm-list">'+(unallocated||emptyState('Fully allocated','No posted economic transaction in this period is missing a job reference.'))+'</div></div></article>';
}
...
function recurringControlView(){
 const control=state.commitments||{},items=control.items||[],active=items.filter(x=>Number(x.active)!==0),archived=items.filter(x=>Number(x.active)===0),summary=control.summary||{},by=control.by_currency||{};
 const cards=Object.entries(by).map(([c,x])=>'<div class="fm-kpi"><span>'+esc(c)+' · committed outflow</span><strong>'+nativeMoney(x.monthly_equivalent,c)+'/mo</strong><small>'+nativeMoney(x.annual_commitment,c)+'/yr · '+num(x.active_outgoing)+' outgoing</small></div><div class="fm-kpi"><span>'+esc(c)+' · subscriptions</span><strong>'+num(x.subscriptions)+'</strong><small>Active subscription records</small></div>').join('');
 const row=r=>{
  const live=Number(r.active)!==0,bank=r.bank_evidence||{};
  const price=bank.possible_price_change?'<small class="bad">Possible bank charge change '+(bank.price_change_percent===null?'':(num(bank.price_change_percent)>=0?'+':'')+num(bank.price_change_percent).toFixed(1)+'%')+'</small>':'';
  const deadline=r.cancellation_deadline?' · cancel by '+date(r.cancellation_deadline):'';
  return '<div class="fm-row"><div><h3>'+esc(r.name||'Recurring item')+'</h3><p>'+esc(r.item_type||'BILL')+' · '+esc(r.frequency||'')+(r.counterparty?' · '+esc(r.counterparty):'')+' · '+esc(r.essentiality||'UNKNOWN')+'</p><small>Next '+date(r.next_due_date)+(r.renewal_date?' · renewal '+date(r.renewal_date):'')+deadline+'</small>'+price+'</div><div class="fm-row-right"><b>'+nativeMoney(r.amount,r.currency||'AUD')+'</b>'+statusBadge(r.control_status||r.lifecycle_decision||'KEEP')+'<small>'+nativeMoney(r.annualized_cost||0,r.currency||'AUD')+'/yr · '+esc(r.lifecycle_decision||'KEEP')+'</small><div class="fm-inline-actions"><button data-recurring-control="'+esc(r.id)+'">Control</button>'+(live?'<button data-recurring-complete="'+esc(r.id)+'">Mark completed</button><button data-recurring-active="0" data-recurring-id="'+esc(r.id)+'">Archive</button>':'<button data-recurring-active="1" data-recurring-id="'+esc(r.id)+'">Restore</button>')+'</div></div></div>';
 };
 return resourceError('commitments','Recurring Commitment Control')+
 '<div class="fm-control-intro"><p>SUBSCRIPTION & RECURRING COMMITMENT CONTROL</p><h2>Control renewals, cancellation deadlines and recurring cost before money leaves</h2><span>Bank-history price changes are suggestions only. Nothing is paid, repriced, renewed or cancelled automatically.</span><div class="fm-control-quick"><button data-personal-new="recurring">+ Recurring item</button><button data-viewjump="calendar">Cash-flow calendar</button><button data-viewjump="forecast">Forecast</button></div></div>'+
 '<div class="fm-grid four"><div class="fm-kpi"><span>Active commitments</span><strong>'+num(summary.active)+'</strong><small>'+num(summary.subscriptions)+' subscription(s)</small></div><div class="fm-kpi"><span>Renewing ≤30 days</span><strong>'+num(summary.renewal_30)+'</strong><small>'+num(summary.cancellation_deadline_30)+' cancellation deadline(s)</small></div><div class="fm-kpi"><span>Decision review</span><strong>'+num(summary.review)+'</strong><small>'+num(summary.cancel_planned)+' cancel-planned</small></div><div class="fm-kpi"><span>Possible price changes</span><strong>'+num(summary.possible_price_change)+'</strong><small>Heuristic bank evidence only</small></div>'+cards+'</div>'+
 '<div class="fm-grid two"><article class="fm-card"><div class="fm-pad"><div class="fm-card-head"><div><h2>Active commitments</h2><p>Saved schedule + renewal/cancellation decision + recent bank evidence.</p></div><button data-personal-new="recurring">+ Add</button></div><div class="fm-list">'+(active.map(row).join('')||emptyState('No active recurring items','Add bills, subscriptions or recurring income to start.'))+'</div></div></article><article class="fm-card"><div class="fm-pad"><div class="fm-card-head"><div><h2>Archived commitments</h2><p>History retained. Restoring does not create or restart a payment.</p></div></div><div class="fm-list">'+(archived.map(row).join('')||emptyState('No archived recurring items','Archived commitments remain available here.'))+'</div></div></article></div>'+
 '<div class="fm-state"><strong>Control boundary</strong><p>'+esc(control.rule||'This module never pays, renews or cancels a service automatically.')+' '+esc(control.bank_evidence_rule||'Bank evidence is suggestion-only.')+'</p></div>';
}
function openRecurringControlForm(id){
 const r=(state.commitments?.items||[]).find(x=>String(x.id)===String(id));if(!r){notice('Recurring commitment is not loaded.',true);return}
 $('fmModalEyebrow').textContent='RECURRING COMMITMENT CONTROL';$('fmModalTitle').textContent='Control · '+(r.name||'Recurring item');
 $('fmModalBody').innerHTML='<form id="recurringControlForm" class="fm-form"><p class="fm-helper">This saves planning/control metadata only. It cannot cancel a service, renew a contract, change the recurring amount or move money.</p><div class="fm-form-grid"><label>Decision<select name="lifecycle_decision">'+['KEEP','REVIEW','CANCEL_PLANNED'].map(v=>'<option '+(String(r.lifecycle_decision||'KEEP')===v?'selected':'')+'>'+v.replaceAll('_',' ')+'</option>').join('')+'</select></label><label>Essentiality<select name="essentiality">'+['UNKNOWN','ESSENTIAL','DISCRETIONARY'].map(v=>'<option '+(String(r.essentiality||'UNKNOWN')===v?'selected':'')+'>'+v+'</option>').join('')+'</select></label></div><div class="fm-form-grid"><label>Auto-renew<select name="auto_renew">'+['UNKNOWN','YES','NO'].map(v=>'<option '+(String(r.auto_renew||'UNKNOWN')===v?'selected':'')+'>'+v+'</option>').join('')+'</select></label><label>Renewal date<input name="renewal_date" type="date" value="'+esc(String(r.renewal_date||'').slice(0,10))+'"></label></div><div class="fm-form-grid"><label>Contract end date<input name="contract_end_date" type="date" value="'+esc(String(r.contract_end_date||'').slice(0,10))+'"></label><label>Cancellation notice days<input name="cancellation_notice_days" type="number" min="0" max="365" value="'+esc(r.cancellation_notice_days??0)+'"></label></div><label>Cancellation / contract reference<textarea name="cancellation_reference" maxlength="500" placeholder="Cancellation URL, contract reference, provider instructions...">'+esc(r.cancellation_reference||'')+'</textarea></label><label>Decision note<textarea name="decision_note" maxlength="700" placeholder="Why keep/review/cancel?">'+esc(r.decision_note||'')+'</textarea></label>'+(r.bank_evidence?.latest?'<div class="fm-state"><strong>Recent bank evidence</strong><p>'+date(r.bank_evidence.latest.date)+' · '+nativeMoney(r.bank_evidence.latest.amount,r.bank_evidence.latest.currency)+' · '+esc(r.bank_evidence.latest.match_confidence||'')+(r.bank_evidence.possible_price_change?' · possible change '+num(r.bank_evidence.price_change_percent).toFixed(1)+'%':'')+'. This evidence will not overwrite your schedule.</p></div>':'')+'<div class="fm-form-actions"><button type="button" data-modal-cancel="1">Cancel</button><button class="primary">Save control</button></div></form>';
 $('fmModal').showModal();document.querySelector('[data-modal-cancel]')?.addEventListener('click',()=>$('fmModal').close());
 $('recurringControlForm').onsubmit=async e=>{e.preventDefault();const body=Object.fromEntries(new FormData(e.currentTarget).entries());try{const x=await api(API+'/personal-money/recurring/'+encodeURIComponent(id)+'/control',{method:'PUT',body:JSON.stringify(body)});$('fmModal').close();notice(x.message);await loadResource('commitments',API+'/personal-money/commitments-control');state.view='recurring';render()}catch(error){notice(error.message,true)}};
}
function savingsControlView(){
 const control=state.savingsReserve||{},goals=control.goals||state.personalAttention?.goals||[],summary=control.summary||{},by=control.by_currency||{};
 const cards=Object.entries(by).map(([c,x])=>'<div class="fm-kpi"><span>'+esc(c)+' · saved progress</span><strong>'+nativeMoney(x.current,c)+'</strong><small>Target '+nativeMoney(x.target,c)+' · '+nativeMoney(x.remaining,c)+' remaining</small></div><div class="fm-kpi"><span>'+esc(c)+' · funding pace</span><strong>'+nativeMoney(x.recent_monthly_pace,c)+'/mo</strong><small>'+nativeMoney(x.planned_monthly_target,c)+'/mo planned · '+num(x.active_goals)+' active</small></div>').join('');
 const rows=goals.map(g=>{
   const target=num(g.target_amount),current=num(g.current_amount),remaining=Math.max(0,num(g.remaining_amount??target-current)),pct=target>0?Math.min(100,current/target*100):0;
 ...
 '<div class="fm-grid four"><div class="fm-kpi"><span>Active goals</span><strong>'+num(summary.active)+'</strong><small>'+num(summary.paused)+' paused · '+num(summary.completed)+' completed</small></div><div class="fm-kpi"><span>Behind pace</span><strong>'+num(summary.behind)+'</strong><small>'+num(summary.overdue)+' overdue target(s)</small></div><div class="fm-kpi"><span>Emergency funds</span><strong>'+num(summary.emergency_funds)+'</strong><small>'+num(summary.emergency_coverage_below_3m)+' below 3 months recorded-spend coverage</small></div><div class="fm-kpi"><span>Control boundary</span><strong>Planning only</strong><small>No automatic money movement</small></div>'+cards+'</div>'+
 '<article class="fm-card"><div class="fm-pad"><div class="fm-card-head"><div><h2>Goal & reserve funding plan</h2><p>Current progress, recent contribution pace, required target pace, reserve type and completion forecast.</p></div><button data-personal-new="savings">+ Add</button></div><div class="fm-list">'+(rows||emptyState('No savings goals','Create an emergency fund, sinking fund or another goal and track it without pretending cash moved.'))+'</div><div class="fm-state"><strong>Calculation boundary</strong><p>'+esc(control.pace_rule||'Recent pace uses recorded goal contributions only.')+' '+esc(control.emergency_rule||'Emergency coverage uses recorded Personal Money spending in the same currency.')+'</p></div></div></article>';
}
function openSavingsControlForm(id){
 const g=(state.savingsReserve?.goals||[]).find(x=>String(x.id)===String(id));if(!g){notice('Savings control record is not loaded.',true);return}
 $('fmModalEyebrow').textContent='SAVINGS & RESERVE CONTROL';$('fmModalTitle').textContent='Control · '+(g.name||'Savings goal');
 $('fmModalBody').innerHTML='<form id="savingsControlForm" class="fm-form"><p class="fm-helper">This saves planning metadata only. It does not transfer funds or change goal progress.</p><div class="fm-form-grid"><label>Goal / reserve type<select name="goal_type">'+['EMERGENCY_FUND','SINKING_FUND','PURCHASE','TRAVEL','INVESTMENT_RESERVE','TAX_RESERVE','OTHER'].map(v=>'<option value="'+v+'" '+(String(g.goal_type||'OTHER')===v?'selected':'')+'>'+v.replaceAll('_',' ')+'</option>').join('')+'</select></label><label>Funding strategy<select name="funding_strategy">'+['MANUAL','WEEKLY_PLAN','FORTNIGHTLY_PLAN','MONTHLY_PLAN'].map(v=>'<option value="'+v+'" '+(String(g.funding_strategy||'MANUAL')===v?'selected':'')+'>'+v.replaceAll('_',' ')+'</option>').join('')+'</select></label></div><div class="fm-form-grid"><label>Planned monthly target ('+esc(g.currency||'AUD')+')<input name="monthly_target" inputmode="decimal" value="'+esc(g.monthly_target||0)+'"></label><label>Protected floor ('+esc(g.currency||'AUD')+')<input name="protected_floor" inputmode="decimal" value="'+esc(g.protected_floor||0)+'"></label></div><div class="fm-form-grid"><label>Liquidity priority<select name="liquidity_priority">'+['LOW','MEDIUM','HIGH'].map(v=>'<option '+(String(g.liquidity_priority||g.priority||'MEDIUM')===v?'selected':'')+'>'+v+'</option>').join('')+'</select></label><label>Preferred contribution day<input name="contribution_day" type="number" min="1" max="31" value="'+esc(g.contribution_day??'')+'" placeholder="1–31"></label></div><label>Funding source note<input name="funding_source_note" maxlength="500" value="'+esc(g.funding_source_note||'')+'" placeholder="Planning reference only, e.g. salary surplus"></label><label>Decision note<textarea name="decision_note" maxlength="700" placeholder="Why this target/priority/pace?">'+esc(g.decision_note||'')+'</textarea></label><div class="fm-state"><strong>Current evidence</strong><p>'+nativeMoney(g.current_amount,g.currency)+' of '+nativeMoney(g.target_amount,g.currency)+' · recent pace '+nativeMoney(g.recent_monthly_pace||0,g.currency)+'/mo · required '+(g.required_monthly_to_target===null||g.required_monthly_to_target===undefined?'—':nativeMoney(g.required_monthly_to_target,g.currency)+'/mo')+'.</p></div><div class="fm-form-actions"><button type="button" data-modal-cancel="1">Cancel</button><button class="primary">Save control</button></div></form>';
 $('fmModal').showModal();document.querySelector('[data-modal-cancel]')?.addEventListener('click',()=>$('fmModal').close());
 $('savingsControlForm').onsubmit=async e=>{e.preventDefault();const body=Object.fromEntries(new FormData(e.currentTarget).entries());try{const x=await api(API+'/personal-money/goals/'+encodeURIComponent(id)+'/control',{method:'PUT',body:JSON.stringify(body)});$('fmModal').close();notice(x.message);await loadResource('savingsReserve',API+'/personal-money/savings-control');state.view='savings';render()}catch(error){notice(error.message,true)}};
}
async function savingsStatement(id){
 try{
  const data=await api(API+'/personal-money/goals/'+encodeURIComponent(id)+'/statement'),g=data.goal||{},tot=data.totals||{},rows=(data.contributions||[]).map(x=>'<div class="fm-row"><div><h3>'+date(x.contributed_at)+'</h3><p>'+esc(x.note||'Recorded savings progress')+'</p></div><b>'+nativeMoney(x.amount,g.currency||tot.currency||'AUD')+'</b></div>').join('');
  openDrawer(g.name||'Savings statement','<div class="fm-grid four"><div class="fm-kpi"><span>Target</span><strong>'+nativeMoney(tot.target_amount,g.currency||tot.currency)+'</strong></div><div class="fm-kpi"><span>Current progress</span><strong>'+nativeMoney(tot.current_progress,g.currency||tot.currency)+'</strong></div><div class="fm-kpi"><span>Remaining</span><strong>'+nativeMoney(tot.remaining_amount,g.currency||tot.currency)+'</strong></div><div class="fm-kpi"><span>Recorded contributions</span><strong>'+nativeMoney(tot.recorded_contributions,g.currency||tot.currency)+'</strong></div></div><article class="fm-card"><div class="fm-pad"><p class="fm-helper">'+esc(data.statement_rule||'Owner-only savings-progress statement.')+' '+esc(data.movement_rule||'No automatic money movement.')+'</p><div class="fm-list">'+(rows||emptyState('No recorded contributions','Add goal progress when you want to record a contribution.'))+'</div></div></article>','SAVINGS & RESERVE STATEMENT');
 }catch(error){notice(error.message,true)}
}
function taxEvidenceControlView(){
 const tax=state.personalTaxControl||{},life=state.assetLifecycle||{},receipts=state.receiptCenter||{},quality=state.quality||{},counts=tax.counts||{};
 const receiptCounts=receipts.counts||{},docs=life.documents||[],expiring=docs.filter(d=>d.days_until_expiry!==null&&d.days_until_expiry!==undefined&&num(d.days_until_expiry)<=30).length;
 const cats=tax.categories||[],review=tax.review_items||[],questions=tax.accountant_questions||[];
 const catRows=cats.slice(0,20).map(c=>'<div class="fm-row"><div><h3>'+esc(c.category||'Uncategorised')+'</h3><p>'+num(c.count||0)+' review item(s) · '+esc(c.currency||'AUD')+'</p></div><b>'+nativeMoney(c.amount,c.currency||'AUD')+'</b></div>').join('');
 const reviewRows=review.slice(0,80).map(r=>'<div class="fm-row"><div><h3>'+esc(r.category||r.entry_type||'Review item')+'</h3><p>'+date(r.occurred_at)+' · '+esc(r.counterparty||r.wallet_name||'')+' · '+esc(r.evidence_status||'UNKNOWN')+'</p>'+(r.accountant_question?'<small>'+esc(r.accountant_question)+'</small>':'')+'</div><div class="fm-row-right"><b>'+nativeMoney(r.amount,r.currency||'AUD')+'</b>'+statusBadge(r.review_status||'UNREVIEWED')+'<button data-personal-tax-review="'+esc(r.id)+'">Review</button></div></div>').join('');
 const questionRows=questions.slice(0,30).map(q=>'<div class="fm-row"><div><h3>'+esc(q.category||'Accountant question')+'</h3><p>'+date(q.occurred_at)+' · '+esc(q.counterparty||'')+'</p><small>'+esc(q.question||'Review requested with accountant.')+'</small></div><b>'+nativeMoney(q.amount,q.currency||'AUD')+'</b></div>').join('');
 return resourceError('personalTaxControl','Tax & Evidence Control')+resourceError('assetLifecycle','Evidence lifecycle')+
 '<div class="fm-control-intro"><p>PERSONAL TAX & EVIDENCE CONTROL</p><h2>Persistent review, evidence status and accountant questions for year-end preparation</h2><span>This organises Personal Money records only. It does not decide deductibility, calculate taxable income or tax payable, create journals, or submit anything to the ATO.</span><div class="fm-control-quick"><button data-viewjump="receipts">Receipt Centre</button><button data-viewjump="reports">GST / reports</button><button data-viewjump="handover">Accountant Handover</button></div></div>'+
 '<div class="fm-grid four"><div class="fm-kpi"><span>Financial year</span><strong>'+esc(tax.financial_year?.label||'Current FY')+'</strong><small>1 July → 30 June preparation</small></div><div class="fm-kpi"><span>Unreviewed</span><strong>'+num(counts.unreviewed)+'</strong><small>'+num(counts.review_candidates)+' review candidate(s)</small></div><div class="fm-kpi"><span>Missing evidence</span><strong>'+num(counts.missing_evidence)+'</strong><small>'+num(counts.has_evidence)+' marked with evidence</small></div><div class="fm-kpi"><span>Ask accountant</span><strong>'+num(counts.ask_accountant)+'</strong><small>Persistent question queue</small></div></div>'+
 '<div class="fm-grid two"><article class="fm-card"><div class="fm-pad"><div class="fm-card-head"><div><h2>Review categories</h2><p>Recorded Personal expense/cash-out activity grouped for preparation only.</p></div></div><div class="fm-list">'+(catRows||emptyState('No tax review categories','No Personal Money review data was returned for this financial year.'))+'</div></div></article><article class="fm-card"><div class="fm-pad"><div class="fm-card-head"><div><h2>Readiness controls</h2><p>Evidence and ledger-quality gaps before accountant handover.</p></div></div><div class="fm-list"><div class="fm-row"><div><h3>Unclassified bank transactions</h3><p>Classification affects reporting quality.</p></div><b>'+num(quality.unclassified)+'</b></div><div class="fm-row"><div><h3>Unreconciled transactions</h3><p>Reconciliation evidence still incomplete.</p></div><b>'+num(quality.unreconciled)+'</b></div><div class="fm-row"><div><h3>Receipt exceptions</h3><p>Missing + requested protected receipt evidence.</p></div><b>'+(num(receiptCounts.missing)+num(receiptCounts.requested))+'</b></div><div class="fm-row"><div><h3>Private evidence due/expired</h3><p>Owner-private document lifecycle references.</p></div><b>'+expiring+'</b></div></div></div></article></div>'+
 '<div class="fm-grid two"><article class="fm-card"><div class="fm-pad"><div class="fm-card-head"><div><h2>Persistent review queue</h2><p>Review state is stored in the protected owner-only Finance backend, not browser localStorage.</p></div></div><div class="fm-list">'+(reviewRows||emptyState('No tax review items','No Personal expense/cash-out candidate was returned.'))+'</div></div></article><article class="fm-card"><div class="fm-pad"><div class="fm-card-head"><div><h2>Accountant questions</h2><p>Items deliberately held for professional review.</p></div></div><div class="fm-list">'+(questionRows||emptyState('No accountant questions','Mark a review item Ask Accountant to build this queue.'))+'</div></div></article></div>'+
 '<div class="fm-state"><strong>Tax boundary</strong><p>'+esc(tax.jurisdiction_note||'This view is preparation only.')+' '+esc(tax.review_rule||'Review state does not create a deduction.')+' '+esc(tax.currency_rule||'Currencies remain separate.')+'</p></div>';
}
function openPersonalTaxReviewForm(id){
 const r=(state.personalTaxControl?.review_items||[]).find(x=>String(x.id)===String(id));if(!r){notice('Tax review item is not loaded.',true);return}
 $('fmModalEyebrow').textContent='PERSONAL TAX PREPARATION';$('fmModalTitle').textContent='Review · '+(r.category||r.counterparty||'Personal Money item');
 $('fmModalBody').innerHTML='<form id="personalTaxReviewForm" class="fm-form"><p class="fm-helper">This stores preparation status only. It does not create a tax deduction, calculate tax, post a journal or file anything.</p><div class="fm-form-grid"><label>Review status<select name="review_status">'+['UNREVIEWED','CANDIDATE','NOT_CLAIMING','ASK_ACCOUNTANT'].map(v=>'<option '+(String(r.review_status||'UNREVIEWED')===v?'selected':'')+'>'+v.replaceAll('_',' ')+'</option>').join('')+'</select></label><label>Evidence status<select name="evidence_status">'+['UNKNOWN','HAS_EVIDENCE','MISSING_EVIDENCE'].map(v=>'<option '+(String(r.evidence_status||'UNKNOWN')===v?'selected':'')+'>'+v.replaceAll('_',' ')+'</option>').join('')+'</select></label></div><div class="fm-state"><strong>Recorded item</strong><p>'+date(r.occurred_at)+' · '+nativeMoney(r.amount,r.currency||'AUD')+' · '+esc(r.counterparty||r.wallet_name||'')+'. Categories are organisational labels only.</p></div><label>Review note<textarea name="review_note" maxlength="1000" placeholder="What did you check?">'+esc(r.review_note||'')+'</textarea></label><label>Question for accountant<textarea name="accountant_question" maxlength="1000" placeholder="Required when status is Ask Accountant">'+esc(r.accountant_question||'')+'</textarea></label><div class="fm-form-actions"><button type="button" data-modal-cancel="1">Cancel</button><button class="primary">Save review</button></div></form>';
 $('fmModal').showModal();document.querySelector('[data-modal-cancel]')?.addEventListener('click',()=>$('fmModal').close());
 $('personalTaxReviewForm').onsubmit=async e=>{e.preventDefault();const body=Object.fromEntries(new FormData(e.currentTarget).entries());try{const x=await api(API+'/personal-money/tax-control/items/'+encodeURIComponent(id),{method:'PUT',body:JSON.stringify(body)});$('fmModal').close();notice(x.message);await loadResource('personalTaxControl',API+'/personal-money/tax-control');state.view='taxcontrol';render()}catch(error){notice(error.message,true)}};
}
...
function cashView(){
 ...
 `<div class="fm-grid two"><article class="fm-card"><div class="fm-pad"><div class="fm-card-head"><div><h2>Cash count & Daily Close</h2><p>Expected recorded balance versus observed physical cash, with custodian and variance review.</p></div><button id="cashCountNewInline">+ Count</button></div><div class="fm-list">${countRows||emptyState('No cash counts yet','Record an actual count to establish daily-close evidence.')}</div></div></article><article class="fm-card"><div class="fm-pad"><div class="fm-card-head"><div><h2>Bank ↔ Cash transfer links</h2><p>Whole Personal withdrawal/deposit transactions only. Original bank evidence remains immutable.</p></div><button id="cashTransferNewInline" ${candidates.length?'':'disabled'}>+ Link</button></div><div class="fm-list">${transferRows||emptyState('No linked cash transfers',candidates.length?candidates.length+' candidate bank movement(s) are available for review.':'No unlinked Personal bank withdrawal/deposit candidate is available.')}</div></div></article></div>`+cashCustodyPanel();
}

function cashCustodyPanel(){
 const data=state.cashCustody||{},cases=data.cases||[],events=data.events||[],summary=data.by_currency||{},byCase={};
 events.forEach(e=>(byCase[e.custody_uid] ||= []).push(e));
 const kpis=Object.entries(summary).map(([c,x])=>'<div class="fm-kpi"><span>'+esc(c)+' custody outstanding</span><strong>'+nativeMoney(x.outstanding_total,c)+'</strong><small>'+num(x.open_cases)+' open · '+num(x.overdue_cases)+' overdue</small></div>').join('');
 const rows=cases.slice(0,50).map(c=>{
  const status=String(c.display_status||c.status||'OPEN').toUpperCase();
  const eventRows=(byCase[c.custody_uid]||[]).slice(0,6).map(e=>'<div class="fm-subrow"><span>'+esc(String(e.event_type||'EVENT').replaceAll('_',' '))+' · '+date(e.occurred_at)+(e.reference_text?' · '+esc(e.reference_text):'')+'</span><b>'+nativeMoney(e.amount,c.currency)+'</b>'+(e.reversed_at?statusBadge('REVERSED'):['RETURN','EXPENSE_CLEARANCE'].includes(e.event_type)?'<button data-cash-custody-event-reverse="'+esc(e.event_uid)+'">Reverse</button>':'')+'</div>').join('');
  const active=!['CLOSED','CANCELLED'].includes(status)&&num(c.outstanding_amount)>0.0001;
  const actions=active?'<button data-cash-custody-return="'+esc(c.custody_uid)+'">Return cash</button><button data-cash-custody-clear="'+esc(c.custody_uid)+'">Clear posted expense</button>':'';
  const cancel=num(c.active_event_count||0)===0&&!['CLOSED','CANCELLED'].includes(status)?'<button class="bad" data-cash-custody-cancel="'+esc(c.custody_uid)+'">Cancel mistaken issue</button>':'';
  return '<div class="fm-row"><div><h3>'+esc(c.custodian||'Custodian')+'</h3><p>'+esc(c.account_name||c.wallet_name||'Cash source')+' · '+esc(c.purpose||'Cash custody')+' · issued '+date(c.issued_at)+' · due '+date(c.due_at)+'</p>'+(eventRows?'<div class="fm-sublist">'+eventRows+'</div>':'')+'</div><div class="fm-row-right"><b>'+nativeMoney(c.outstanding_amount,c.currency)+' outstanding</b><small>issued '+nativeMoney(c.issued_amount,c.currency)+' · returned '+nativeMoney(c.returned_amount,c.currency)+' · expense-cleared '+nativeMoney(c.cleared_expense_amount,c.currency)+'</small>'+statusBadge(status)+'<div class="fm-inline-actions">'+actions+cancel+'</div></div></div>';
 }).join('');
 return '<article class="fm-card"><div class="fm-pad"><div class="fm-card-head"><div><h2>Cash Custody & Petty Cash Register</h2><p>Tracks cash given to a person, cash returned, and already-posted expenses cleared against that float. Custody itself does not alter the ledger.</p></div><button id="cashCustodyNewInline">+ Issue custody</button></div><div class="fm-grid four">'+(kpis||'<div class="fm-kpi"><span>Custody outstanding</span><strong>—</strong><small>No active custody.</small></div>')+'</div><div class="fm-list">'+(rows||emptyState('No cash currently in custody','Issue controlled custody when cash leaves the till/safe but remains under company or owner control.'))+'</div><p class="fm-helper">'+esc(data.expense_rule||'Expense clearance links only already-posted expenses; it never duplicates spending.')+'</p></div></article>';
}
function cashCustodySources(){
 const control=state.cashControl||{};
 return [
  ...(control.cash_accounts||[]).map(a=>({value:'BANK_ACCOUNT:'+a.id,label:(a.ownership_scope||'ACCOUNT')+' · '+a.nickname+' · '+nativeMoney(a.expected_balance??a.available_balance??a.current_ledger_balance,a.currency)})),
  ...(control.personal_wallets||[]).map(w=>({value:'PERSONAL_WALLET:'+w.id,label:'PERSONAL · '+w.name+' · '+nativeMoney(w.balance,w.currency)}))
 ];
}
function openCashCustodyIssueForm(){
 const sources=cashCustodySources();if(!sources.length){notice('Create a Cash/Petty Cash account or Personal cash wallet first.',true);return}
 $('fmModalEyebrow').textContent='CASH CUSTODY';$('fmModalTitle').textContent='Issue cash to custodian';
 $('fmModalBody').innerHTML='<form id="cashCustodyIssueForm" class="fm-form"><p class="fm-helper">This records physical custody only. It does not create an expense or change the cash ledger.</p><label>Cash source<select name="source_ref" required>'+sources.map(s=>'<option value="'+esc(s.value)+'">'+esc(s.label)+'</option>').join('')+'</select></label><div class="fm-form-grid"><label>Custodian / staff member<input name="custodian" maxlength="160" required></label><label>Amount issued<input name="amount" inputmode="decimal" required></label></div><div class="fm-form-grid"><label>Issued at<input name="issued_at" type="datetime-local" value="'+localIsoDay()+'T'+new Date().toTimeString().slice(0,5)+'" required></label><label>Expected return / clearance by<input name="due_at" type="datetime-local"></label></div><label>Purpose<input name="purpose" maxlength="240" placeholder="Petty cash float, site purchase float..."></label><label>Note<textarea name="note" maxlength="700"></textarea></label><div class="fm-form-actions"><button type="button" data-modal-cancel="1">Cancel</button><button class="primary">Issue custody</button></div></form>';
 $('fmModal').showModal();document.querySelector('[data-modal-cancel]')?.addEventListener('click',()=>$('fmModal').close());
 $('cashCustodyIssueForm').onsubmit=async e=>{e.preventDefault();const fd=new FormData(e.currentTarget),parts=String(fd.get('source_ref')||'').split(':');try{const x=await api(API+'/cash-control/custody',{method:'POST',body:JSON.stringify({source_type:parts[0],source_id:parts.slice(1).join(':'),custodian:fd.get('custodian'),amount:fd.get('amount'),issued_at:fd.get('issued_at'),due_at:fd.get('due_at'),purpose:fd.get('purpose'),note:fd.get('note')})});$('fmModal').close();notice(x.message);await Promise.all([loadResource('cashCustody',API+'/cash-control/custody'),loadResource('cashControl',API+'/cash-control')]);state.view='cash';render()}catch(error){notice(error.message,true)}};
}
async function cashCustodyReturn(uid){
 const amount=prompt('Cash amount returned:');if(amount===null||!String(amount).trim())return;
 const note=prompt('Return note / reference (optional):')||'';
 try{const x=await api(API+'/cash-control/custody/'+encodeURIComponent(uid)+'/return',{method:'POST',body:JSON.stringify({amount,note})});notice(x.message);await Promise.all([loadResource('cashCustody',API+'/cash-control/custody'),loadResource('cashControl',API+'/cash-control')]);render()}catch(error){notice(error.message,true)}
}
async function cashCustodyClearExpense(uid){
 try{
  const data=await api(API+'/cash-control/custody/'+encodeURIComponent(uid)+'/expense-candidates'),candidates=data.candidates||[];
  if(!candidates.length){notice('No eligible posted expense is available to clear against this custody record.',true);return}
  $('fmModalEyebrow').textContent='CASH CUSTODY';$('fmModalTitle').textContent='Clear posted expense against custody';
  $('fmModalBody').innerHTML='<form id="cashCustodyExpenseForm" class="fm-form"><p class="fm-helper">This links an expense that already exists in the ledger. It will not create a second expense.</p><label>Posted expense<select name="candidate" required>'+candidates.map(c=>'<option value="'+esc(c.candidate_type+':'+c.id)+'">'+esc(date(c.occurred_at)+' · '+c.description+' · '+nativeMoney(c.amount,c.currency))+'</option>').join('')+'</select></label><label>Note<textarea name="note" maxlength="500"></textarea></label><div class="fm-form-actions"><button type="button" data-modal-cancel="1">Cancel</button><button class="primary">Link expense</button></div></form>';
  $('fmModal').showModal();document.querySelector('[data-modal-cancel]')?.addEventListener('click',()=>$('fmModal').close());
  $('cashCustodyExpenseForm').onsubmit=async e=>{e.preventDefault();const fd=new FormData(e.currentTarget),parts=String(fd.get('candidate')||'').split(':');try{const x=await api(API+'/cash-control/custody/'+encodeURIComponent(uid)+'/clear-expense',{method:'POST',body:JSON.stringify({candidate_type:parts[0],candidate_id:parts.slice(1).join(':'),note:fd.get('note')})});$('fmModal').close();notice(x.message);await Promise.all([loadResource('cashCustody',API+'/cash-control/custody'),loadResource('cashControl',API+'/cash-control')]);render()}catch(error){notice(error.message,true)}};
 }catch(error){notice(error.message,true)}
}
...
function openBulkReview(){
 ...
 applyButton.onclick=async()=>{if(!preview||!body)return;applyButton.disabled=true;try{const result=await api(I+'/transactions/bulk/review',{method:'POST',body:JSON.stringify({...body,preview:false,expected_count:Number(preview.affected_count)})});$('fmModal').close();state.selectedTransactions.clear();notice(result.message);financeDataChanged();await loadTransactions()}catch(error){applyButton.disabled=false;notice(error.message,true)}};
}
...
function simpleView(v){
 ...
 if(v==='profitability')return jobProfitabilityView();
 if(v==='counterparties')return counterpartyControlView();
...
function bindDynamic(){
 ...
 document.querySelectorAll('[data-personal-tax-review]').forEach(b=>b.onclick=()=>openPersonalTaxReviewForm(b.dataset.personalTaxReview));
 ...
 document.querySelectorAll('[data-savings-control]').forEach(b=>b.onclick=()=>openSavingsControlForm(b.dataset.savingsControl));
 document.querySelectorAll('[data-savings-statement]').forEach(b=>b.onclick=()=>savingsStatement(b.dataset.savingsStatement));
 document.querySelectorAll('[data-goal-contribute]').forEach(b=>b.onclick=()=>openGoalContributionForm(b.dataset.goalContribute));
 document.querySelectorAll('[data-recurring-control]').forEach(b=>b.onclick=()=>openRecurringControlForm(b.dataset.recurringControl));
 ...
 document.querySelectorAll('[data-statement-edit]').forEach(b=>b.onclick=()=>openStatementVaultEditor(b.dataset.statementEdit));
...
async function transactionDetail(id){
 try{
  ...
  setTimeout(()=>{
   ...
   document.querySelector('[data-category-move-open]')?.addEventListener('click',()=>openTransactionCategoryMove(id,r));
...
function openNew(kind='expense',presetAccount=''){
 ...
 $('fmModal').showModal();
 document.querySelector('[data-modal-cancel]')?.addEventListener('click',()=> $('fmModal').close());
 $('fmEntryForm').onsubmit=async e=>{e.preventDefault();try{const x=await saveManualMovement(e.currentTarget);$('fmModal').close();notice(x.message);financeDataChanged();await refresh()}catch(error){notice(error.message,true)}};
}
