...
function currencyRows(){
 ...
 const economic=Array.isArray(state.dash?.economic_by_currency)?state.dash.economic_by_currency:[];
 return [...new Set([...balances.map(x=>x.currency),...flows.map(x=>x.currency),...economic.map(x=>x.currency)].filter(Boolean))].map(currency=>{
  const e=economic.find(x=>x.currency===currency)||{};
  return {
   currency,
   ...
   economic_income:num(e.economic_income),
   net_economic_expense:num(e.net_economic_expense),
   refund_offset:num(e.refund_offset),
   reimbursement_settlement:num(e.reimbursement_settlement),
...
function hero(){
 ...
 const kpis=rows.length?rows.map(r=>`<div class="fm-kpi"><span>${esc(r.currency)} · Available balance</span><strong>${nativeMoney(r.balance,r.currency)}</strong><small>Current balance — not period filtered</small></div><div class="fm-kpi"><span>${esc(r.currency)} · Cash in</span><strong class="good">${nativeMoney(r.money_in,r.currency)}</strong><small>Selected period · transfers excluded</small></div><div class="fm-kpi"><span>${esc(r.currency)} · Cash out</span><strong class="bad">${nativeMoney(r.money_out,r.currency)}</strong><small>Selected period · transfers excluded</small></div><div class="fm-kpi"><span>${esc(r.currency)} · Net cash flow</span><strong>${nativeMoney(r.net_flow,r.currency)}</strong><small>Cash in minus cash out</small></div><div class="fm-kpi"><span>${esc(r.currency)} · Economic income</span><strong class="good">${nativeMoney(r.economic_income,r.currency)}</strong><small>Linked refunds excluded from revenue</small></div><div class="fm-kpi"><span>${esc(r.currency)} · Net economic expense</span><strong class="bad">${nativeMoney(r.net_economic_expense,r.currency)}</strong><small>Refunds netted · reimbursement settlements excluded</small></div>`).join(''):'<div class="fm-empty">No visible financial activity.</div>';
