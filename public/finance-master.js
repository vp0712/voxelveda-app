...
function settingsView(){
 const caps=state.capabilities?.capabilities||{},cfg=state.companySettings?.settings||{};
 const form='<article class="fm-card"><div class="fm-pad"><div class="fm-card-head"><div><h2>Company Finance Settings</h2><p>Used by accountant-ready reports where supported. Changes are permission and step-up protected.</p></div></div><form id="companySettingsForm" class="fm-form"><div class="fm-form-grid"><label>Legal name<input name="company_legal_name" value="'+esc(cfg.company_legal_name||'Voxel Veda Pty Ltd')+'"></label><label>Trading name<input name="trading_name" value="'+esc(cfg.trading_name||'Voxel Veda')+'"></label></div><label>Address<input name="company_address" value="'+esc(cfg.company_address||'')+'"></label><div class="fm-form-grid"><label>Email<input name="company_email" type="email" value="'+esc(cfg.company_email||'info@voxelveda.com')+'"></label><label>Phone<input name="support_phone" value="'+esc(cfg.support_phone||'')+'"></label></div><div class="fm-form-grid"><label>ABN<input name="abn" value="'+esc(cfg.abn||'')+'"></label><label>Website<input name="website" value="'+esc(cfg.website||'https://voxelveda.com')+'"></label></div><div class="fm-form-grid"><label>Base currency<input name="base_currency" maxlength="3" value="'+esc(cfg.base_currency||state.setup?.settings?.default_currency||'AUD')+'"></label><label>Financial year start<input name="financial_year_start" placeholder="07-01" value="'+esc(cfg.financial_year_start||'07-01')+'"></label></div><label>Report footer<input name="report_footer" value="'+esc(cfg.report_footer||'Confidential Financial Information')+'"></label><div class="fm-form-actions"><button class="primary">Save company settings</button></div></form></div></article>';
 return form+connectionsView()+'<article class="fm-card"><div class="fm-pad"><div class="fm-card-head"><div><h2>Capability Registry</h2><p>Unsupported workflows are explicitly identified rather than presented as dead buttons.</p></div></div><div class="fm-capability-grid">'+Object.entries(caps).map(([k,v])=>'<div class="fm-capability"><div><b>'+esc(k.replaceAll('_',' '))+'</b><p>'+esc(v.note||'')+'</p></div>'+statusBadge(v.status)+'</div>').join('')+'</div></div></article>';
}
...
function bindDynamic(){
 ...
 document.querySelectorAll('[data-notification-pref]').forEach(c=>c.onchange=async()=>{try{await api('/api/notifications/preferences',{method:'PATCH',body:JSON.stringify({category:c.dataset.notificationPref,in_app_enabled:c.checked})});notice('Notification preference saved.')}catch(error){c.checked=!c.checked;notice(error.message,true)}});
