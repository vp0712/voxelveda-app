...
const canonicalFinanceUiAssets = new Set([
  ...
  'finance-profitability-control.js'
]);
...
app.get('/admin',noIndex,pageAuth({workspaceOnly:true}),renderAdminPage);app.get('/finance-intelligence',noIndex,pageAuth({workspaceOnly:true}),sendPage('finance-intelligence.html'));app.get('/banking',noIndex,pageAuth(),redirectPreservingQuery('/finance-intelligence'));app.get('/finance-reconciliation',noIndex,pageAuth(),(req,res)=>res.redirect(302,'/finance-intelligence#reconciliation'));app.get('/dashboard',noIndex,pageAuth(),sendPage('staff-dashboard.html'));app.get('/profile',noIndex,pageAuth(),sendPage('profile.html'));app.get('/employee-id',noIndex,pageAuth({workspaceOnly:true}),sendPage('employee-id.html'));app.get('/quality',noIndex,pageAuth({workspaceOnly:true}),sendPage('quality.html'));app.get('/shop-floor',noIndex,pageAuth(),sendPage('shop-floor.html'));app.get('/invoice/view',noIndex,pageAuth(),sendPage('invoice-pdf.html'));
