...
const { corsOptions, csrfProtection, enforceHttps, rateLimit, safeApiResponses, securityHeaders, safeErrorHandler } = require('./middleware/securityMiddleware');
const auth = require('./middleware/auth');

const app = express();
const publicDir = path.join(__dirname, 'public');
const noStorePublicAssets = new Set(['login.js','admin-dashboard.js','staff.js','auth-lifecycle.js','mfa.js','security-page.js','step-up.js','step-up.css','style.css','advanced-theme.css','mobile-shell.js','quality.js','quality.css','shop-floor.js','shop-floor.css','service-worker.js']);
...
app.use(rateLimit());
app.use(express.json({limit:process.env.JSON_BODY_LIMIT||'1mb',type:['application/json','application/csp-report','application/reports+json'],verify(req,res,buffer){req.rawBody=Buffer.from(buffer)}}));
...
const protectedModuleRoutes=['/rfqs','/invoices','/customers','/suppliers','/stock','/raw-material','/packaging','/expenses','/workforce','/timesheets','/roster','/staff','/finance','/financial-years','/compliance','/forms','/settings','/meetings','/tasks','/trash'];
