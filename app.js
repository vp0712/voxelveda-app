...
const noStorePublicAssets = new Set(['login.js','admin-dashboard.js','staff.js','profile.js','procurement-ui.js','auth-lifecycle.js','mfa.js','security-page.js','step-up.js','step-up.css','style.css','advanced-theme.css','mobile-shell.js','quality.js','quality.css','shop-floor.js','shop-floor.css','service-worker.js']);
...
app.get('/',sendPage('index.html'));app.get('/login',noIndex,sendPage('login.html'));app.get('/register',noIndex,sendPage('register.html'));app.get('/request-quote',sendPage('customer.html'));app.get('/privacy',sendPage('privacy-policy.html'));app.get('/terms',sendPage('terms.html'));app.get('/support',sendPage('support.html'));app.get('/forgot-password',noIndex,sendPage('forgot-password.html'));app.get('/reset-password',noIndex,sendPage('reset-password.html'));app.get('/accept-invite',noIndex,sendPage('accept-invite.html'));app.get('/mfa',noIndex,sendPage('mfa.html'));app.get('/security',noIndex,pageAuth({allowMfaSetup:true}),sendPage('security.html'));app.get('/attendance-terminal',noIndex,sendPage('shift-qr.html'));app.get('/401',noIndex,(req,res)=>res.status(401).sendFile(path.join(publicDir,'401.html')));app.get('/403',noIndex,(req,res)=>res.status(403).sendFile(path.join(publicDir,'403.html')));app.get('/404',noIndex,(req,res)=>res.status(404).sendFile(path.join(publicDir,'404.html')));app.get('/429',noIndex,(req,res)=>res.status(429).sendFile(path.join(publicDir,'429.html')));app.get('/500',noIndex,(req,res)=>res.status(500).sendFile(path.join(publicDir,'500.html')));app.get('/maintenance',noIndex,(req,res)=>res.status(503).sendFile(path.join(publicDir,'maintenance.html')));
app.get('/admin',noIndex,pageAuth({workspaceOnly:true}),sendPage('admin-dashboard.html'));app.get('/dashboard',noIndex,pageAuth(),sendPage('staff-dashboard.html'));app.get('/profile',noIndex,pageAuth(),sendPage('profile.html'));app.get('/quality',noIndex,pageAuth({workspaceOnly:true}),sendPage('quality.html'));app.get('/shop-floor',noIndex,pageAuth(),sendPage('shop-floor.html'));app.get('/invoice/view',noIndex,pageAuth(),sendPage('invoice-pdf.html'));
app.get('/index.html',redirectPreservingQuery('/'));app.get('/login.html',redirectPreservingQuery('/login'));app.get('/register.html',redirectPreservingQuery('/register'));app.get('/customer.html',redirectPreservingQuery('/request-quote'));app.get('/privacy-policy.html',redirectPreservingQuery('/privacy'));app.get('/admin-dashboard.html',redirectPreservingQuery('/admin'));app.get('/staff-dashboard.html',redirectPreservingQuery('/dashboard'));app.get('/profile.html',redirectPreservingQuery('/profile'));app.get('/quality.html',redirectPreservingQuery('/quality'));app.get('/shop-floor.html',redirectPreservingQuery('/shop-floor'));app.get('/dashboard.html',redirectPreservingQuery('/dashboard'));app.get('/invoice-pdf.html',redirectPreservingQuery('/invoice/view'));app.get('/shift-qr.html',redirectPreservingQuery('/attendance-terminal'));
app.get('/approvals',noIndex,pageAuth(),(req,res)=>{const portal=['admin','super_admin'].includes(String(req.user?.role||'').toLowerCase())?'/admin':'/dashboard';return res.redirect(302,`${portal}?view=approvals`)});
...
app.get(protectedModuleRoutes,noIndex,pageAuth(),(req,res)=>{const routeName=req.path.replace(/^\//,'');const portal=hasAnyPermission(req.user,['VIEW_FINANCE','MANAGE_JOBS','MANAGE_TEAM_JOBS','VIEW_CUSTOMERS','VIEW_INVENTORY','VIEW_SUPPLIERS','VIEW_RFQS'])?'/admin':'/dashboard';return res.redirect(302,`${portal}?view=${encodeURIComponent(routeName)}`)});
...
app.post('/api/public/rfq',rateLimitPolicy('public_rfq'),publicRfqContract,botChallenge('public_rfq'),publicSubmissionDedupe({submissionType:'public_rfq'}),rfqController.createRFQ);app.post('/api/public/ai-lead',rateLimitPolicy('ai_lead'),aiLeadContract,botChallenge('ai_lead'),publicSubmissionDedupe({submissionType:'public_ai-lead'}),aiLeadController.createLead);app.get('/api/public/shift-qr',rateLimitPolicy('shift_qr'),shiftQrContract,attendanceController.publicShiftQrToken);app.get('/api/public/qms/coc/:no/verify',rateLimitPolicy('coc_verification'),cocVerificationContract,qualityReleaseController.verifyCoc);app.get('/api/qms/coc/:no/verify',rateLimitPolicy('coc_verification'),cocVerificationContract,qualityReleaseController.verifyCoc);app.use('/api/integrations/webhooks',integrationWebhookRoutes);
app.use('/api/rfq',auth,requirePermission('VIEW_RFQS'),rfqRoutes);app.use('/api/invoice',auth,requirePermission('VIEW_FINANCE'),invoiceRoutes);app.use('/api/users',auth,userRoutes);app.use('/api/profile',auth,profileRoutes);app.use('/api/settings',auth,settingsRoutes);app.use('/api/email',auth,requirePermission('MANAGE_COMPANY_EMAIL'),emailRoutes);app.use('/api/dashboard',auth,dashboardRoutes);app.use('/api/upload',auth,uploadRoutes);app.use('/api/documents',auth,documentSecurityRoutes);app.use('/api/notifications',auth,notificationRoutes);app.use('/api/trash',auth,trashRoutes);app.use('/api/workflows',auth,workflowRoutes);app.use('/api/security/readiness',auth,readinessRoutes);app.use('/api/security/workers',auth,backgroundJobRoutes);app.use('/api/security',auth,securityDashboardRoutes);app.use('/api/security',auth,securityIncidentRoutes);app.use('/api/security/operations',auth,operationalTrustRoutes);app.use('/api/security/assurance',auth,continuousAssuranceRoutes);app.use('/api/security/governance',auth,securityGovernanceRoutes);app.use('/api/tasks',auth,taskRoutes);app.use('/api/stock',auth,requirePermission('VIEW_INVENTORY'),stockRoutes);app.use('/api/customers',auth,requirePermission('VIEW_CUSTOMERS'),requireInputPermission('EDIT_CUSTOMERS'),customerRoutes);app.use('/api/materials',auth,requirePermission('VIEW_INVENTORY'),materialRoutes);app.use('/api/meetings',auth,requirePermission('VIEW_MEETINGS'),meetingRoutes);app.use('/api/roster',auth,requirePermission('VIEW_ATTENDANCE'),rosterRoutes);app.get('/api/suppliers/files/:id/view',auth,requirePermission('VIEW_SUPPLIERS'),supplierController.viewSupplierFile);app.use('/api/suppliers',auth,requirePermission('VIEW_SUPPLIERS'),supplierRoutes);app.use('/api/expenses',auth,requirePermission('VIEW_FINANCE'),expenseRoutes);app.use('/api/finance',auth,requirePermission('VIEW_FINANCE'),financeRoutes);app.use('/api/high-risk-finance',auth,highRiskFinanceRoutes);app.use('/api/qms',auth,qmsRoutes);app.use('/api/compliance',auth,requirePermission('VIEW_COMPLIANCE'),requireInputPermission('EDIT_COMPLIANCE'),complianceRoutes);app.use('/api/competitors',auth,requirePermission('VIEW_CUSTOMERS'),requireInputPermission('EDIT_CUSTOMERS'),competitorRoutes);app.use('/api/access-attempts',auth,require('./routes/accessAttemptRoutes'));
...
app.use((req,res)=>{if(!req.path.startsWith('/api/')&&req.accepts('html'))return res.status(404).sendFile(path.join(publicDir,'404.html'));return res.status(404).json({message:'Route not found'})});app.use(safeErrorHandler);module.exports=app;
